...
feature_status = waymo.feature_status   # defined next to the data path it reads
...
def main():
    ...
    ap.add_argument("--split", default="val", help="status: which split to count")
    a = ap.parse_args()
    if a.steps == "status":  # no RunLog: this is polled every few minutes by the extraction loop
        st = feature_status(a.set, a.split)
        print(" ".join(str(st[k]) for k in ("on_disk", "indexed", "built", "frames_indexed", "of")))
        return
