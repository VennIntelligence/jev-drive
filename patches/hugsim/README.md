...
| `optional/lqr-tracker-v2.patch` | NOT applied by default; only in the private `fixed2` tree (on top of the PR #57 fix), the candidate controller of the acceptance. `traj2control` resamples the plan to 0.25 s and the iLQR discretization becomes 0.25 s (the simulator step, upstream issue #75); steering-rate input cost 10 -> 1; no 50 ms wall-clock cap. |
