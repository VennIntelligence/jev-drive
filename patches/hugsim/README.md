...
| `optional/ideal-tracker.patch` | NOT applied by default; only in the private `ideal` tree of `scripts/hugsim/zs_run.py`, the reference for controller acceptance (todos/2026-09-25-closed-loop-infra-acceptance/hugsim-controllers.md). `closed_loop.py` skips `traj2control` and sends the plan's own motion over one step (quadratic through the origin and the first two waypoints); `HUGSimEnv.step` moves the ego exactly there when the action carries `pose`. Collision, route and scoring code are untouched. |
