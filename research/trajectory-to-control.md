...
状态：2026-09-23修订。v1–v4与正式60条完成，无合格替代默认；后续真实TCP纵向六例有局部行为信号但障碍失败未改善，独立转弯六例候选未通过逐弯验收。
...
真实TCP保持checkpoint、原生四点/2s轨迹、20Hz推理和横向仲裁，仅在共同执行限幅下比较纵向原生/PI，完整六例现已结束。两臂都完成24211、1711，1773都在34.88%发生一次车辆碰撞后耗尽4000tick。PI在前两条有局部速度/平顺性信号，但障碍案例接触更早、停滞更久；三路线全程均值下降不能替代这项安全失败。原生标签物理原点仍待确认，未造出5s轨迹来接入本节的完整pursuit。
...
[后续总报告](../todos/2026-09-23-tcp-controller/final-report.md)保存协议、正负结果、静止负速度接入修复、原始索引与每版图表。横向下一步先查重规划/定位扰动与转向变化的关系，再冻结一个机制明确的改动，不靠自动扫参获得好看的均值。
