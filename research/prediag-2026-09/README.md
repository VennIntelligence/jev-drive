...
| P3 | backbone 阶梯 | 表征换谁 | (a) 32B null；(d)(d′) V-JEPA 家族已测，方向分歧未收敛，等 train 训裁决；(b) H3 deferred；(c) Wan 抽取待重跑（hub 检查断网）；(d″) Qwen 原生视频已排 |
...
- 表征：时间要长进表征里，不能拼在读出端；JEPA 式的视频表征是第一个候选，但它在可信九档上的增益只在一个方向成立，两方向都成立的只有第 10 档朝被打分 proposal 偏好的移动；生成式（H3、Wan）待比；
