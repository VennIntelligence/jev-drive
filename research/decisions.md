...
## 40. 冻结特征 + 薄 head 上，openpilot 的时序特征比所有通用 backbone 都好；Alpamayo 1.5 的 VLM 中层特征不是（openpilot 部分**已定**，WOD + nuScenes 两个数据集；Alpamayo 部分**待定**）
...
3. 增益的形状：DiD 为正（+0.16 到 +0.30），大头在直行帧（纵向：前车、停车、起步）；第 10 档对 rater_best 的 ADE −1.0 到 −1.6 m，向 rater 偏好的一支挪。
   ...
   **2026-09-25 修正**：这 0.5 RFS 原先整体算作「读出层还有空间」，现在量过了——把同一个 `temporal` 换进分类头（`cls ego` 那一族，train 训，同 479 帧），
   ...
   Cinque 与 8.00 仍差 −0.29 [−0.47, −0.10]，按 seed 0 判「没补上」（**2026-09-26 R40 改判，见下**：3 seed 都没补上，不随 seed 变）。
   代价照例是 ADE（pre-onset +0.18 / +0.21）。
   *2026-09-26 补 seed（[夜间队列](../todos/2026-09-26-overnight-queue.md) 第 2 项，seed = k-means 词表 + 内层划分，其余不变）*：`cls_late` 的 RFS 在 3 个 seed 上是 7.64 / 7.50 / 7.50（Cinque）、7.73 / 7.46 / 7.57（Lebowski），
   对 `cls ego` 的增益 +0.38 / +0.19 / +0.29 与 +0.43 / +0.17 / +0.34，极差（0.18 / 0.26）超过原 CI 半宽（0.17 / 0.16）；`--seed 0` 重跑与 seed 0 在已知的 GPU k-means / L-BFGS 非确定性范围内一致（cluster mean 差 0.01–0.05，[SEEDS] 队列 01:31 已定性），所以这个极差是词表 seed 的效应，不是 run-to-run 噪声。
   `ridge_late` 各行是确定性的（3 seed 逐位相同），pre-onset 的 −0.29 / −0.32 m 不受影响。
   **2026-09-26 就地修正（R40，按第 40 条原判据的 3 seed 重算完成，代码、判据一字不改，[结果](results/real-data-transfer/r40/README.md)）**：上面「线性分类头补上了缺口的 0.4–0.7」和「Lebowski 够到原生」都只是 seed 0 一次的读数；
   原文曾把 3 seed 重算记为「还没做」，现在补上。`cls_late` − 原生的配对差（frame mean，sequence bootstrap CI）与 G 在 seed 0（上面这次 run）/ seed 1 / seed 2 上：
   Cinque −0.29 [−0.47, −0.10]（G 0.40）/ −0.46 [−0.66, −0.26]（G 0.05）/ −0.42 [−0.60, −0.23]（G 0.13），三个 seed 都判「没补上」；
   Lebowski −0.10 [−0.29, +0.09]（G 0.70，**够到原生**）/ −0.35 [−0.56, −0.16]（G −0.06，没补上）/ −0.23 [−0.42, −0.04]（G 0.31，没补上）。
   **Cinque 不随 seed 变，三个 seed 都没补上；Lebowski「够到原生」只在 seed 0 上成立，是否够到原生随词表 seed 变。**
   G 在两模型三个 seed 共 6 个点上从 **−0.06 到 0.70**（Cinque 均值 0.19、Lebowski 均值 0.32），原来写的「0.4–0.7」只是 seed 0 一次的读数，不是稳定范围。
   三个 seed 的逐帧预测取平均后按同一套代码重算（3-seed ensemble，不是任何单个 seed 的读数）：Cinque −0.26 [−0.43, −0.07]（对 ridge_late 的 CI 不跨零，判「补上部分缺口」）、
   Lebowski −0.11 [−0.29, +0.07]（判「够到原生」）。
...
**状态**：openpilot 部分**已定**（2026-09-25，由待定升级）：原先的限定是「半 val 子集、一个 split、一个数据集」，现在 train 训协议（WOD 全 val）和一个独立数据集（nuScenes，不同城市、相机、司机）
都按预登记规则判「更好」，量级按相对值一致。仍然成立的限定：每个协议只有一个 seed；nuScenes 上 pre-onset 的证据只在不带 command 的设定里；openpilot 的训练视频与两个数据集没有已知重叠（推测）。
Alpamayo 部分仍**待定**：`L27_last` 在 train 训协议上的复现（约 20 h 抽取）要用户决定。读出层的下一步是多模态解码（diffusion head 在 41.5 万行上约 2.3 h 满载 GPU，要等一张不和闭环考试共享的卡）。
全部表：[todos/2026-09-24-driving-backbones/README.md](../todos/2026-09-24-driving-backbones/README.md)「第 40 条后续的结果」。
...
**补 seed（2026-09-26，[夜间队列](../todos/2026-09-26-overnight-queue.md) 第 2 项，seed = 路线分折排列）**：配对差分双流在 3 个 seed 上行人翻转 43.3 / 43.6 / 42.4%（Cinque）、41.6 / 39.2 / 41.9%（Lebowski），
三个 seed 都过判据，极差只有原 CI 半宽的 1/6–1/3；hard-example 与均匀对照每个 seed 都是 0–3.7%，null 4.4–5.6%。本条「单 seed」的限定对 M-C 可以去掉。
...
## 43. 融合前诊断：Qwen ⊕ openpilot 在**均匀 imitation 下**冗余、在配对差分下互补；现成 SAM 3.1 近处看得见、远处放不准，规则门在它的状态上的行人翻转已被学出来的 reaction head 超过（**待定**，P5 v0/v1 + WOD + nuScenes；2026-09-26 就地修正）
...
**2026-09-26 就地修正（P5 v1 出数后）**：本条原标题和结论 1、2 写的是「Qwen 在融合里不加分（冗余）→ 去掉 Qwen」和「行人唯一非零的是结构化感知 + 几何规则」。
**两句都要改。** Q1 在 P5 v1 上复跑（`research/results/fusion-diagnostics/q1-v1/`，BA 集 406 个行人 reactive 帧、49 条路线）：均匀 imitation 的拼接与 late fusion 行人翻转仍是 0.2–1.2%、合并翻转比单路 openpilot 低 5–12 pp，
也就是**在均匀 imitation 这个训练信号下**冗余的判格照旧成立；但同一批特征、同一批帧，第 42 条修正后的 M-C（配对差分）行人 43.3% [35.0, 50.7]，hard-example 重加权 0%。
所以 Q1 的预登记规则问错了问题：它把「拼接加不加分」当成特征的性质来测，而结果是训练信号的性质——Qwen 的行人信息一直在，均匀目标激发不出来，配对目标能。
原结论 2 的「唯一非零」也不再成立：学出来的双流 head（43%，非反应帧误刹 11.7%）已经超过 SAM 状态上的规则门（29%，行人 family 31–40%，GT 状态版非反应帧误触发 48%）。
「行人走 SAM 通道」这个判格由此撤回；SAM 的位置改为离线造配对（真实帧反事实编辑）与快通道的检测 embedding 候选，见 [elicitation 计划](../todos/2026-09-26-elicitation-program.md)。
**第 3 阶段补充（2026-09-26，fusion todo 的预登记口径，PDM-Lite 集为主）**：PDM-Lite 集（783 个合并 reactive 帧 / 48 条路线）上所有考生都贴地板——openpilot `ridge_late` 0.5%、TFv6 waypoint 12.5%、
Qwen pooled 配对差分行人 0.6%、Qwen 4×4 网格 4.5% [0, 12.5]（训练对上斜率 0.004）、**GT 几何规则门 4.2%**（BA 集 50.7%）；Q1 concat − best 两个集合都不 > 0。
所以按登记（以 PDM 为准）Q1 仍「冗余」、Q9 仍为 0、Q6 的「GT 规则 floor ≥ 80%」不成立；BA 集（Qwen 网格 46.1%、pooled 42.1%）给出相反的 Q9 判格。
两个 expert 的差别在考卷：PDM-Lite 在因素可见后约 0.4 s 就反应，考的是预判，连特权几何规则都做不到。全表在 todo 的「第 3 阶段」一节。
Q4（SAM 召回与接地点）、Q2（WOD 损失解剖）、Q7、Q8 的读数不受影响。原文保留如下以便对照。

**结论（原文，2026-09-25，已被上面的修正部分推翻）**：
1. 在我们测过的每一个协议上，把 Qwen 拼到 openpilot 上都不加分，P5 上还减分；第 40 条「openpilot `temporal` 是最强冻结表征」延伸为「融合里不需要第二个通用 VLM 表征」。〔2026-09-26：只在均匀 imitation 下成立〕
2. 行人这一条，openpilot（第 42 条 D0）、Qwen pooled（M-C）、Qwen 空间 token（Q9b）三种读出在 P5 v0 上都是 0；**唯一非零的是结构化感知 + 几何规则**（SAM 状态 29%，其中行人 family 31–40%）。〔2026-09-26：v1 上 M-C 43%，已不是唯一〕
...
按登记三个训练集都不同时满足「编辑 / 安慰剂 ≥ 2」与「WOD Pedestrians RFS Δ CI > 0」。限定（2026-09-26 G3 之后就地修正；原写「编辑对信噪比低、WOD 对少（CARLA 上激发用了约 400 个行人 reactive 帧）、标签 (c)（PDM scorer）没做」，三条都被 G3 改写）：
信噪比低只对 pooled 特征成立，不是编辑质量的问题；WOD 对少是真的，但扩量买不到新事件；标签 (c) 已做，不救。

**G3（第二轮，[todos/2026-09-26-real-data-transfer.md](../todos/2026-09-26-real-data-transfer.md) 的 G3 节）：E2 的两项诊断与标签 (c) 重训——E2 仍不过，但瓶颈的位置变了。**
(a) **编辑质量不是瓶颈**（G3a，登记读法：CARLA 上 Qwen 比值 < 2）：同一统计量在 P5 v1 BA 行人对上，Qwen `L18_last` 的去掉 hazard / 天气 null 比 0.23 [0.19, 0.29]。天气 null 比安慰剂大得多，这个比值小于 2 几乎是构造决定的；
有信息的是分子：CARLA 里整个去掉行人让 Qwen 特征移动 0.26 个 x⁺ 标准差，比真实帧上抹人（0.32）还小，openpilot 0.04–0.06 对 0.07–0.08。配对差分在 CARLA 上用这么小的位移就激发出了 43% 的行人翻转，所以 E2 的「只比安慰剂多 21%」是 pooled 全图特征对一个行人的固有灵敏度，视频一致 inpainting 按读法不投。
检测 embedding 上相反：真实抹人的位移是安慰剂的 152 倍 [116, 246]。
(b) **标签 (c) 不救**（G3b）：PDM scorer（v1.1）在 911 对里的 456 对上偏好刹停；用它重训的双流 M-C 更坏（R1 1.75、WOD RFS −2.98、直行激活 51%，三个 seed 逐位相同）。
换成 openpilot ⊕ YOLO embedding 的 student（E5 配方、G0 的真实数据 embedding），编辑对上读得出抹人（R1 6–14，全部过 2 倍线），但读出的修正很小：主判的 student A (c) 在 WOD 上 Δ 中位 6 cm，全部帧 −0.02 [−0.05, +0.01]、Pedestrians −0.03 [−0.09, +0.03]、直行激活 0.1%——无害但没用；
