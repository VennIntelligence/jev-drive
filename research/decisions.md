...
真实CARLA无交互开发集则CARLA和pursuit各4/4通过，TCP在26966失败。合成与实车排序不能混作同一种证据。
Dev10seed0全部10条均有官方结果，三组都9/10完成；pursuit全程路线均值横向RMS=.2951m，CARLA=.2711m，
...
| 新默认与跑榜收益 | 待定 | seed1及保留集正在复跑；route oracle无避障且没有接真实planner，不能宣布分数改进 |

已发现route adapter的当前位置至首点连接段可推高轨迹导数，配置8m/s不代表每帧reference8m/s；
独立G2真值巡航误差与Dev10输入轨迹导数误差必须分开。此限制留在数据里，不通过改指标隐藏。

**怎么才能定下来**：按[冻结验收方案](../todos/2026-09-22-b2d-controller/plan.md)完成两seed与预定6条保留集，
分别保留完成率、控制失败、横向及速度门槛；不满足则不推荐默认替代。真实planner收益另做固定checkpoint的controller-only对照。
