...
第 10 档那一半现在不能这么读**：那一档的 logged future 自己就不被 rater 认可（RFS 5.92、41.7% 被压到下限），
也就是说在那里「对着 log 算 ADE」本身就不是个好目标，增益掉回去有相当一部分是**目标造成的**，
不是表征造成的。**站得住的部分**是第 4 到第 9 档之间的上升和在第 8 档见顶：那一段 logged future 的
...
对 rater_best 的 ADE 从 1.8 m 涨到 7.8 m（4.4 倍）。** 也就是说：在 s_ego 最高的 10% 帧上，
**人类评分者普遍不认同司机当时的选择**。那一档不是「一个难但唯一的正确答案」，是**好几个都说得过去的答案**。
...
MLP（6.651 / 6.370）全部更差。样本量从半 val 的 237/242 涨到 **479**，CI 半宽从约 ±0.3 缩到约 ±0.21，
所以这次不只是「测不到增益」——**在这个精度下，视觉在 RFS 上是持平偏负的**。

**没有改变的**：第 10 档的目标本身有争议（上一节的 rater 检验），所以倒 U 的下降段仍然不能
...
## 24. P3 backbone 阶梯：换表征（**(d) 已测，(a) 抽取中，(b)(c) 待定**）
...
按第 20 条追加的那条读法，第 10 档不参与判定（那一档的 logged future 本身有争议），
...
## 22. 「关键时刻」的评测口径：主判用 s_ego 第 1–9 档的 ADE，RFS 并排，顶档单独报（**提案，待用户接受**）

**这一条是提案，不是结论。** 第 20 条在结尾留下一句「**在换表征之前先把这件事定下来**，
否则换完了也不知道有没有变好」，P1 就是去把它定下来的
（计划见 [todos/2026-09-22-p1-judge.md](../todos/2026-09-22-p1-judge.md)）。
下面的数字已经测了，**推荐的口径要用户拍板**。

**问题**：在 s_ego（ego readout 自己的残差，第 20 条的主定义）最高的那一档上，
**司机自己开出来的 logged future 都不被 rater 认可**（完整 val 479 个 rater 帧的第 10 档：
RFS 5.92、41.7% 被压到下限、对 rater_best 的 ADE 7.82 m）。也就是说，我们最想测的那批帧，
恰恰是「对着 log 算 ADE」最没有意义的地方。而「关键时刻视觉有没有用」这句话的真假，
完全取决于用哪个 judge。
...
1. **视觉在现行口径上的增量，有将近一半来自那个目标本身有争议的顶档。**
...
- **只有它有 power。** pre-onset 上各 judge 的 CI 半宽：`ADE vs log` 是 **0.046–0.094（n=1510）**，
...
那里 logged future 自己 RFS 只有 5.92、41.7% 被压到下限，对着它算位移没有意义。

**(ii) minADE over top-k：不推荐当主判。** 原因见下面一节。
...
**状态**：**待定（提案）。** 数字已经测了，口径要用户接受之后才生效。
**在接受之前，P2 / P3 的结果请同时按两套口径报**（现行的全集 ADE，和这里的第 1–9 档 + RFS），
这样无论最后定哪一套都不用重跑。

**怎么才能定下来**：用户接受或改写上面的推荐。如果要更强的证据，可以再做一件事——
...
真实TCP首轮还暴露了静止附近微小负速度误触reverse guard的问题。修复共同边界后完整重跑六例，未拼接旧成功案例；旧运行、重放、失败和所有图版保留。模型四点/.5s采样、2s时域保持原生，训练标签物理原点仍未确认，未把oracle短前视移植后冒充真实模型横向收益。
...
右弯唯一失败集中在43.23–45.93m晚core连续8帧。独立course（区间后轴运动方向）审查显示，车身/运动方向差只能解释其中一部分，仍有2.53–4.20°运动方向残差；不能事后换指标通过。下一步缩小到这段航向取舍，同时保持原四窗验收。不同物理运行的S基线有最多.0585m位置差，本轮只用自己的配对基线；不把固定输入默认等价当成跨运行完全一致。
