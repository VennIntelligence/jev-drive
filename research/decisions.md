...
## 23. P2 读出阶梯：pooled 单帧之外，readout 和输入各松一格（**(b) 已测，(c)(d)(e) 数字待填**）
...
待填。grid 特征（每相机 token map 池化到 6×8，3 × 48 = 144 个 token × 2560 维，
737 KB/帧，2 万帧约 14.9 GB）的抽取路径**已用 200 帧验证和盘上的 `qwen_front3`
逐位相同**（200/200 行，max |Δ| = 0.0），所以 (c) 和 A 可比这一点不用再担心。
抽取速度实测 **88.7 ms/帧**，峰值显存 9.72 GB。

**状态**：(b) **已确认**（限定在半 val 预演和这批冻结特征上）；(c)(d)(e) 待跑。
