...
- Login: `ssh autodl` (alias in local `~/.ssh/config`, key auth; now `connect.westd.seetacloud.com`, port 41708).
  User `ujs`, five RTX PRO 6000 Blackwell Server Edition (96 GB each, sm_120) since 2026-09-25 15:00 (three from that morning, two from 2026-09-24, one before), driver 595.71, CUDA 13.2. Pin jobs with `CUDA_VISIBLE_DEVICES`.
- Region: West-D (`$AutoDLRegion` = `west-D`). See [storage.md](storage.md) for why it matters.
- CPU and RAM: the host has 208 cores and ~1 TB, but our container's cgroup allows **125 cores**
  (`/sys/fs/cgroup/cpu.max` = `12500000 100000`) and **600 GB RAM** (`memory.max`). `os.cpu_count()` can report
