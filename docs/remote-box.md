...
- Login: `ssh autodl` (alias in local `~/.ssh/config`, key auth; now `connect.westd.seetacloud.com`, port 41708).
  User `ujs`, seven RTX PRO 6000 Blackwell Server Edition (96 GB each, sm_120) since 2026-09-26 12:25 (five from 2026-09-25 15:00), driver 595.71, CUDA 13.2. Pin jobs with `CUDA_VISIBLE_DEVICES`.
- Region: West-D (`$AutoDLRegion` = `west-D`). See [storage.md](storage.md) for why it matters.
- CPU and RAM: the host (Xeon Platinum 8470Q, Sapphire Rapids) has 208 cores and ~1 TB, but our container's cgroup allows **175 cores**
  (`/sys/fs/cgroup/cpu.max` = `17500000 100000`) and **840 GB RAM** (`memory.max`); both scale with the card count (25 cores and 120 GB per card). `os.cpu_count()` can report
...
- Disk: the system disk is 30 GB, keep it empty. The data disk is 3.1 TB. Put everything in `~/data`, see [storage.md](storage.md).
