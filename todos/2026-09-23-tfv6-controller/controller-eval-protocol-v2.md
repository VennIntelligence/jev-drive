...
## 第五处修正：plan 不要求倒退（2026-09-25）

dev 调参第一轮（`tune/round1-result.json`）所有候选都不合格，C/D 在 dev 上也有 8 例失败，几乎全在 profile 参考上、CTE 5–12 m。逐 tick 看 2050/D：参考在中途停车点停下时 D 冲过约 0.3 m，时间索引的固定轨迹于是给出"先后退 0.3 m、停住、再前进"的 plan；plan 前后都有点，"轨迹全在车后"保护不触发，按弧长算的期望速度为正，D 全油门冲出、横向瞄点错乱、驶出路面。真实 planner 不会给倒车再前进的 plan。修正：每个未来时刻的目标弧长取 `max(参考弧长(t), 自车当前在参考路径上的弧长)`（投影单调），超过日程的车被要求原地等参考追上，落后的车 plan 不变。影响所有控制器，held-out L1（`void-*-reverse/`）与 dev 第一轮全部作废重跑。流水线此后把 held-out A/B/C/D 的 L1 与 dev 调参并行跑。

## L1 v3 与候选 P3/P4（2026-09-25）

新增 `crawl` 参考（前 40 m、0.3–1.5 m/s、每 8–20 m 停 2.5–4 s）和接口模式 `model_noise`（每次更新叠加随时域增大的新噪声）、`stale_2hz`、`stale_1hz`。crawl 只截取路线前 40 m，而 validator 的完成判定仍以整条路线终点为准，所以 crawl 的 status 恒为 blocked，只看分数不看完成数。P3 在 2 Hz 下 0/14 完成：车在中途停车后落后日程约 17 m，时间索引参考把追赶位移全部塞进新 plan 的第一段（目标速度 40–68 m/s），前馈变成 −25 到 −50 m/s²，与 jerk 限幅一起把车锁死；PCHIP 把这个跳变摊到邻段，所以 P3 比 P2 更糟。这种首段跳变在真实 planner 里也会出现（TFv6 约 16% 帧属于不可行 plan），因此不改 L1 的锚定方式，而在控制器侧把前馈裁到车辆能力范围 [−4, +3] m/s²（P4 = P3 + 前馈限幅）。

## P7：可行性整形（2026-09-26，看结果前登记）

P5/P6 在 TFv6 闭环的 1825、4183 上失败：模型起步给出约 3 s 内 0→11 m/s 并转弯的不可行 plan，P 系列近乎忠实执行而撞车。P7 = P6 + `accel_request_max_mps2 = 2.0`：正向加速度命令上限 2.0 m/s²，刹车不设限。2.0 取自 L1 ramp 参考的起步加速度上限，登记于看任何 P7 结果之前，不再调。评测：dev 与 held-out L1（名义三类参考 + 八种接口）、TFv6 16 × 2 与 TCP 8 路线闭环，与 P6 同协议。判据：闭环完成数与碰撞不差于 C，且 L1 名义 primary 不劣于 D。
