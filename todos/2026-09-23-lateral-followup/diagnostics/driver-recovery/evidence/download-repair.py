...
url="https://download.nvidia.com/XFree86/Linux-x86_64/580.159.03/NVIDIA-Linux-x86_64-580.159.03.run"
size=398016015
chunk=4194304
(root/'parts').mkdir(exist_ok=True)
log=(root/'download-repair-events.jsonl').open('a',buffering=1)
def download(i):
 start=i*chunk; end=min(size,start+chunk)-1
 p=root/'parts'/str(i)
 if not p.exists() or p.stat().st_size!=end-start+1:
  subprocess.run(['curl','-fsSL','--max-time','90','--retry','5','--retry-all-errors','--speed-limit','32768','--speed-time','15','--range',f'{start}-{end}','-o',str(p),url],check=True)
