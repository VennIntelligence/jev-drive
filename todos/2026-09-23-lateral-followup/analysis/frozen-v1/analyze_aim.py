...
VARIANTS = ('baseline-linear', 'candidate-hermite')
...
def required_conditions(metrics, cases, coverage, recovery):
    ...
    for route in ROUTES:
        for segment in ([1,2] if route=='17563' else [1]):
            ...
            def compare(name,field,band='window',delta=None,ratio=None,minimum=None,reverse=False,strict=False):
                b=index.get((route,'baseline-linear',segment,band),{})
                c=index.get((route,'candidate-hermite',segment,band),{})
            ...
            if route in ('26966','24240'):
                compare('primary_emitted_rate_20percent','steer_rate_per_s_p95_abs',ratio=.8,minimum=.01)
                compare('primary_lateral_jerk_strict_decrease','lateral_jerk_mps3_p95_abs',delta=0.,minimum=.01,strict=True)
            else:
                compare('emitted_rate_p95_ratio','steer_rate_per_s_p95_abs',ratio=1.20,minimum=.01)
                compare('lateral_jerk_p95_ratio','lateral_jerk_mps3_p95_abs',ratio=1.10,minimum=.01)
            if route=='26966':
                compare('post10m_rms_no_increase','cte_m_rms',band='post10m',delta=0.)
                recovery_notes.append(dict(route=route,segment=segment,status='reported_not_a_recovery_pass',
                    baseline=recovery_index.get((route,VARIANTS[0],segment)),candidate=recovery_index.get((route,VARIANTS[1],segment)),
                    reason='Historical baseline censored; fixed post10m residual is gated separately. Censor is never recovery success.'))
            else:
                br=recovery_index.get((route,VARIANTS[0],segment),{});cr=recovery_index.get((route,VARIANTS[1],segment),{})
                covered=all(coverage.get((route,v,segment),{}).get('complete') is True for v in VARIANTS)
                bv,cv=br.get('recovery_time_s'),cr.get('recovery_time_s')
                available=covered and br and cr
                add(f'turn/{route}/{segment}/recovery_no_new_censor',
                    (not br['recovery_censored'] and not cr['recovery_censored']) if available else None,baseline=br,candidate=cr)
                finite=all(isinstance(x,(int,float)) and np.isfinite(x) for x in (bv,cv))
                add(f'turn/{route}/{segment}/recovery_time_delta',cv<=bv+.10+1e-9 if available and finite else None,
                    baseline=bv,candidate=cv,limit=bv+.10 if isinstance(bv,(int,float)) else None)
    ...
    return dict(status='fail' if 'fail' in statuses else ('insufficient' if 'insufficient' in statuses else 'pass'),
        ...
        recovery_notes=recovery_notes,scope='Hermite几何插值开发筛选；两目标窗emitted-rate及physical-jerk收益，逐窗保护；不追认旧15%CTE目标，不代表默认或真实TCP资格')
...
def main():
    ...
    for route in ROUTES:
        ...
        for variant in VARIANTS:
            ...
            expected_mode='linear' if variant==VARIANTS[0] else 'hermite'
    ...
    for name, rows in [('metrics.csv',metrics),('recovery.csv',recovery),('cases.csv',cases),('frames.csv',full_frames)]:
    ...
    manifest = dict(legacy_fixture=args.legacy_fixture,run_root=str(args.run_root.resolve()), inputs=hashes,
        ...
        protocol=dict(windows='Frozen v1: 26966 one,17563 two,24240 one; no error-based reselection',
            ...
            candidate='baseline-linear vs candidate-hermite; both max(3m,.5s*v). Requested/used/fallback preserved separately incl null no evaluation. No old15percentCTE success claim; actual config archived.'))
