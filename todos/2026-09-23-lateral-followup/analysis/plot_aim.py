...
COLORS={'baseline-linear':'#2563a6','candidate-hermite':'#d35f20'}
LABELS={'baseline-linear':'Baseline: linear aim, max(3 m, 0.5 s × speed)',
        'candidate-hermite':'Candidate: Hermite aim, same lookahead'}
...
def main():
    ...
    ap.add_argument('--frozen-helper',type=Path,default=BASE/'analyze_aim.py')
    ...
    data={};references={}
    fixture_label=' | LEGACY ALIAS FIXTURE, NOT LIVE' if analysis_manifest.get('legacy_fixture') else ''
    ...
    for route in ns['ROUTES']:
        ...
        for window in windows[route]:
            ...
            for variant,rows in part.items():
                ...
                axes[2,0].plot(t,[r['rate'] for r in rows],color=color,label='Emitted '+variant)
                axes[2,0].plot(t,[r['raw_rate'] for r in rows],color=color,ls=':',alpha=.6)
                axes[2,1].plot(t,[r['heading'] for r in rows],color=color)
                for level,field,value in [(0,'trajectory_updated',True),(1,'aim_interpolation_used','hermite')]:
                    times=[time for time,r in zip(t,rows) if r.get(field)==value]
                    axes[2,2].scatter(times,[level]*len(times),s=8,color=color,alpha=.5)
                times=[time for time,r in zip(t,rows) if r.get('aim_interpolation_fallback') is not None]
                axes[2,2].scatter(times,[2]*len(times),marker='x',s=18,color=color)
            labels={(0,1):'CTE (m, LEFT positive)',(0,2):'Steer (normalized, RIGHT positive)',
                    ...
                    (2,0):'Steer rate (/s; dotted=raw)',(2,1):'Heading tracking error (deg, RIGHT positive)',
                    (2,2):'Events: 0=update, 1=Hermite, 2=fallback'}
            ...
            axes[2,2].set_yticks([0,1,2]);axes[2,2].set_ylim(-.3,2.3)
            fig.suptitle(f"Route {route}, fixed window {int(window['segment'])}: {window['start_m']:g}–{window['end_m']:g} m\n"
                         'Diagnostic route oracle | policy none | all window frames | raw, unfiltered signals'+fixture_label)
            fig.tight_layout();save(fig,f"route-{route}-window-{int(window['segment'])}")
    for name,source in [('plot-source.py',Path(__file__)),('analysis-source.py',args.frozen_helper)]:
        (args.out/name).write_bytes(source.read_bytes());generated.append(name)
