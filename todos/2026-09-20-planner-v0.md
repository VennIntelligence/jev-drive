...
本次 run 在 box 上 `$DATA_DIR/runs/planner_v0/v1.0-trainval/20260920-111446/`：
`results.{csv,md}`、`paired.csv`（每个对比 × 每个子集一行）、`subsets.csv`、`per_sample.npz`、`vocab_K*.npy`、
...
但结论不是"视觉在 onset 上特别有用"，而是**恰恰相反的一件事**：
...
- 但这些都还**不能当成结论**。ΔADE 的 CI 半宽是 0.031 m，和待测效应本身 0.033 m 同量级；
  要分辨的是"onset 上的增量是否**大于** straight 上的 0.029"，那需要一个
  difference-of-differences，n=135 这边的宽度根本撑不起来，本次没有做这个检验。
  **这个测量 underpowered by construction**：它能排除"onset 上有一个远大于 0.03 m 的增量"，
  其余什么都排除不了。

所以这一格在 nuScenes 上只能记成**未测定**，既不是"视觉在起手前有用"的证据，
也**绝不能**被写成"视觉在起手前没用"的证据。
**这件事只能在 Waymo 上定**——那里同一个子集约有 1750 帧，是本项目最关键的一次测量，
已经按 [decisions 3d](../research/decisions.md) 预登记，计划见
[2026-09-20-waymo-stage-a-dryrun.md](2026-09-20-waymo-stage-a-dryrun.md)。
