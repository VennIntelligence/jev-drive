...
- 所有均值都带 **scene-level bootstrap CI**（按 scene 有放回重采样 1000 次，取 2.5/97.5 分位）；
  两个方法之间的比较用 **paired**（配对）差值的 bootstrap，因为同一批 val 帧过两个 head，
  场景间方差会抵消，比看两条 CI 是否重叠灵敏得多。
...
1. **mean pooling（对 image token 取平均）稳定好于 last token**，回归上差约 0.15 m，分类上差约 0.3 m。
   这和 probe v1 在转向分类上的结论不同（那里深层 last token 的 NLL 最好），说明"读哪个 token"
   要看下游任务：轨迹回归需要空间铺开的信息，转向类别更像一个全局判断。
...
在 `turning` 上视觉的增益还在（miss 0.784 → 0.759，ADE 1.075 → 1.049），和全集差不多。
但在 `onset`（n=135）上，视觉**没有**帮助：ADE 1.331 → 1.297（CI [1.177,1.488] 和 [1.148,1.456]
大幅重叠），miss 甚至从 0.889 变成 0.919。这是 probe v1 结论的反面——那里视觉的增量恰恰集中在
hard subset。

有两个解释，需要分开：(a) n=135 太小，CI 半宽 ±0.15 m，根本测不出 3% 量级的差异；
(b) 转向"要不要起手"是一个类别判断（probe 测的正是这个），而"起手之后 3 s 内走多远、
转多少度"仍然由速度决定，位移指标测不到那个类别信息。**(b) 是推测**，验证方法是在 onset 子集上
同时报 probe 的转向 macro-F1 和这里的 ADE，看两者是否脱钩；Waymo 上 onset 样本会多得多
（479 个 rater 帧全是 long-tail 场景），应该先在那里重做这张表再下结论。
...
- **onset 子集只有 135 个样本**，任何关于"视觉在 maneuver 起手前有没有用"的结论在 nuScenes 上都测不动。
  这条必须在 Waymo 上重做。
