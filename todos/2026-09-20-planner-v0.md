...
本次 run 在 box 上 `$DATA_DIR/runs/planner_v0/v1.0-trainval/20260920-113447/`（latency / bench 的数字来自上一轮 `20260920-111446`，那部分代码没动）：
`results.{csv,md}`、`paired.csv`（每个对比 × 每个子集一行）、`did.csv`、`subsets.csv`、`per_sample.npz`、`vocab_K*.npy`、
`latency.json`、`bench.json`，以及两张图的 PDF 和 PNG。整条链路 14.0 min（plan 13.0 min，bench 1.1 min）。
...
从 3.780 降到 3.642（paired −0.163 m，CI [−0.286, −0.032]，显著），但 miss 只降 0.007（不显著），
...
|:--|--:|--:|:--|:--|
| ridge → cls，`L22_mean` | 0.864 → 0.769 | 3.268 → 3.787 | **−0.096 [−0.136, −0.060]** | **+0.519 [+0.262, +0.753]** |
| ridge → cls，ego state | 0.367 → 0.364 | 0.772 → 0.850 | −0.003 [−0.033, +0.028] | +0.079 [+0.050, +0.107] |
| cls → cls_soft，`L22_mean` | 0.769 → 0.763 | 3.787 → 3.624 | −0.005 [−0.021, +0.012] | −0.163 [−0.286, −0.032] |

在视觉 feature 上，**固定词表分类比连续回归的 ADE 差 0.52 m（约 16%），但 miss 低 9.6 个百分点**，
...
| ego cls → late fusion 加视觉（分类侧） | −0.001 [−0.004, +0.001] | +0.003 [−0.000, +0.007] |
...
下面是 late fusion 相对 ego-only ridge 的配对差值（同一批帧过两个 head，按 scene bootstrap，
负号表示变好），数据来自本次 run 的 `paired.csv`：

| 子集 | n | Δmiss [95% CI] | ΔADE (m) [95% CI] |
|:--|--:|:--|:--|
| all | 4356 | −0.029 [−0.052, −0.012] | −0.029 [−0.040, −0.019] |
| turning | 877 | −0.025 [−0.052, −0.001] | −0.026 [−0.047, −0.006] |
| **onset** | **135** | **+0.030 [+0.000, +0.065]** | **−0.033 [−0.065, −0.003]** |
| straight | 3479 | −0.030 [−0.057, −0.008] | −0.029 [−0.041, −0.018] |

**配对检验推翻了只看两行区间时的读法。** 上一节两行的 CI（1.331 [1.177,1.488] 对
1.297 [1.148,1.456]）大幅重叠，看上去"视觉在 onset 上没用"；但配对之后
ΔADE = −0.033，CI [−0.065, −0.003] **不跨零**——场景间方差抵消掉之后，位移上的增量是真的。
这正是为什么**任何比较性主张都必须引配对差值**：重叠的边缘区间什么都不说明。

**但真正要问的问题是 difference-of-differences（DiD）**：onset 上的增量**是否大于** straight 上的增量？
这才是"增量集中在 ego prior 最弱处"这个框架成立与否的判据，一个显著的 onset delta 推不出它。
两个子集共享 scene，所以 bootstrap 必须用同一次重采样同时作用在两边（`traj.boot_did`）：

| 对比 | onset 的 Δ | straight 的 Δ | **DiD [95% CI]（半宽）** |
|:--|--:|--:|:--|
| ego ridge → late fusion，ADE (m) | −0.033 | −0.029 | **−0.004 [−0.038, +0.032]（0.035）** |
| ego ridge → late fusion，miss | +0.030 | −0.030 | **+0.060 [+0.020, +0.105]（0.043）** |
| ego cls → late fusion，ADE (m) | −0.009 | +0.001 | −0.010 [−0.034, +0.009]（0.022） |

**位移上的 DiD 是 −0.004 m，点估计基本是零，CI 半宽 0.035 m 是它的 9 倍。**
也就是说：增量确实是**均匀**的，没有任何证据表明它集中在 onset；同时这个测量也**分辨不了
任何小于约 0.035 m 的集中效应**。这两句话必须一起说。

信任域口径上 DiD 是 **+0.060，CI [+0.020, +0.105]，显著为正**——相对 straight 而言，
onset 上加视觉让预测**更容易**被判出界。这是一个方向明确的反向信号，不过 nuScenes 的 miss
是围着单条 logged future 建的 stand-in，不能直接外推到 Waymo 的 RFS。

所以结论不是"视觉在 onset 上特别有用"，而是**恰恰相反的一件事**：

- ΔADE 在四个子集上几乎一样（−0.029 / −0.026 / −0.033 / −0.029）。
  **视觉的增量是均匀的，没有集中在 ego prior 最弱的地方。** probe v1 那个
  "增量集中在 hard subset"的模式，在位移口径下**没有**复现。
- 信任域口径下 onset 甚至是反的：Δmiss = **+0.030**（变差），CI 下界压在 0 上，
  而 straight 是 −0.030。也就是说在"还没起手但要转"的帧上，加视觉让预测更容易被判出界。
- 但这些都还**不能当成结论**。DiD 的点估计是 −0.004 m 而 CI 半宽 0.035 m，
  **这个测量 underpowered by construction**：它只能排除"onset 上有一个比 straight 大 0.035 m 以上的集中效应"，
  其余什么都排除不了。顺带一个对 Waymo 有用的标定：DiD 的半宽（0.035）只比 onset delta 的半宽
  （0.031）大 1.13 倍，不是朴素的 √2 倍——因为对照臂 straight 有 3479 帧，几乎不贡献方差。
  按 √n 缩放到 Waymo 的 1750 帧、再乘 Waymo 约 3 倍的 ADE 尺度，DiD 的半宽约 **0.029 m**，
  比 [decisions 3d](../research/decisions.md) 里 0.037 m 的预估略好一点。

所以这一格在 nuScenes 上只能记成**未测定**，既不是"视觉在起手前有用"的证据，
也**绝不能**被写成"视觉在起手前没用"的证据。
**这件事只能在 Waymo 上定**——那里同一个子集约有 1750 帧，是本项目最关键的一次测量。
已经按 [decisions 3d](../research/decisions.md) 预登记，**主量就是 DiD**，
计划见 [2026-09-20-waymo-stage-a-dryrun.md](2026-09-20-waymo-stage-a-dryrun.md)。

另外一个仍待验证的**推测**：转向"要不要起手"是一个类别判断（probe 测的正是这个），
而"起手之后 5 s 内走多远、转多少度"仍然由速度决定，位移指标可能测不到那个类别信息。
...
- **onset 子集只有 135 个样本**，ΔADE 的 CI 半宽 0.031 m 而效应本身 0.033 m，
  **underpowered by construction**。配对检验说位移上的增量不跨零，但四个子集的增量几乎一样，
  "增量是否集中在 onset"这个真正要问的问题需要 difference-of-differences，n=135 撑不起来。
  任何关于"视觉在 maneuver 起手前有没有用"的说法（正反都算）在 nuScenes 上都不成立。
  这条必须在 Waymo 上重做（那里约 1750 帧），已按 decisions 3d 预登记。
...
## 补充：把对照 backbone 提前到 nuScenes 上（2026-09-20 下午）

目的不是 nuScenes 的数字（那仍然是彩排），而是**在 Waymo val 落地之前，把抽取路径、代价和坑先踩一遍**，
免得到时候第一次遇到。[decisions 12](../research/decisions.md) 定的三个对照是
DINOv2（已有）、**V-JEPA 2**（视频自监督）、SigLIP2（图文预训练）。

### SigLIP2：单帧，直接进现有路径

`google/siglip2-so400m-patch14-384`，取 `pooled`（模型自带的 attention pooling head）和 `patch_mean`。
输入按它预训练的方式**压成 384×384 的正方形**，不保持长宽比——保持的话就得 padding，那不是它见过的东西。
（DINOv2 那边我们是保长宽比 resize 到 252×448 的，两者不同，要写进实验细节。）

### V-JEPA 2：它要 clip，而我们只有 2 Hz——这才是真正要解决的问题

`facebook/vjepa2-vitl-fpc64-256` 每个 clip 要 **64 帧**。而我们只抽了 `samples/CAM_FRONT`，
2 Hz，3.5 s 的历史只有 7 张图，根本喂不满。解决办法是把 **`sweeps/CAM_FRONT`** 也解出来。

**代价（从盘上的元数据数出来的，不是估的）**：trainval 的 CAM_FRONT 一共 **198 315** 帧，
其中 keyframe 34 149、sweeps **164 166**。按 samples 实测的 143 KB/帧（4.9 GB / 34 149），
sweeps 约 **23.5 GB**；盘上还有 589 GB。时间上，绑定成本是**把 10 个 blob 归档从头读一遍**
（tar.gz 不可 seek），上次抽 `samples/CAM_FRONT` 用了 **3547 s（59 min）**，这次读的是同样的 314 GB，
所以也是 **约 1 h**，纯 CPU + 磁盘，**不占网络也不占 GPU**。

**结论：完全合理，已经开跑**（`scripts/tmux_run.sh sweeps scripts/extract_nuscenes.sh trainval sweeps/CAM_FRONT`）。
抽完之后 CAM_FRONT 从 2 Hz 变成 **约 12 Hz**，64 帧的 clip 正好覆盖 **5.25 s** 真实历史
（63 × 83.3 ms），也就是 V-JEPA 2 的原生配置。

**踩到的坑（会原样出现在 Waymo 上）**：第一版把"这个 clip 算不算完整"的容差定成**请求步长的一半**
（5.25/64/2 = 41 ms），结果 mini 上只有 152/404 通过，远低于 scene 长度允许的 ~290。
原因是请求的间隔（83.3 ms）和相机的原生间隔（~84–100 ms）**几乎相等但不完全相等**，
64 个槽位一路累积相位漂移，半步的容差在结构上就不可能一直成立。
改成：容差取**从数据里量出来的原生步长**，并且"完整"的判据改成**整个跨度落在 scene 内**——
后者才是我们真正在乎的事，槽位上 ±40 ms 的抖动对视频模型无所谓。改完 mini 是 **294/404（73%）**，
和 scene 长度算出来的一致。

| clip 跨度 | mini 上完整的 keyframe | 备注 |
|--:|--:|:--|
| 5.25 s（64 帧 @ 原生帧率） | 294 / 404（73%） | 默认，V-JEPA 2 原生配置 |
| 3.50 s | 331 / 404（82%） | 和我们打算给 Stage B 的窗口一致 |
| 2.00 s | 362 / 404（90%） | |

**Waymo 上要注意的同一件事**：那边的 shard 是被打散的子集，`waymo.history_rows` 也有一条容差规则，
同样会撞上"请求间隔 ≈ 原生间隔"这个陷阱。**步长要从数据里量，不要假设。**

### 公平性：整张表都要限制到 clip 完整的帧

一个被 padding 撑起来的 clip，并没有真的看到我们给它记的那段历史。所以 `planner_v0.py` 新增
`--clip-set vjepa2`：**把整张表（baseline、ego、Qwen、全部）都限制到 clip 完整的那些行**，
而不是只约束 V-JEPA 2 自己那一行。

### V-JEPA 2 那一行测的到底是什么

**它和 history 天然混淆**（[decisions 12](../research/decisions.md)）：它比单帧 backbone 多拿了 5.25 s。
赢了也可能整个是 history 效应而不是 backbone 效应，唯一能分开的办法是给 Qwen 同样的窗口，那是 Stage B。
所以这一行只能读成**"在 Stage A 的代价下，喂时间信息值不值"**，不是 backbone 排名。
这句话写在 `features.py` 的文件头，不留给写作时再补。

### 现在的状态

- 代码就绪：`features.py` 里 `SiglipFeatures` / `VJepaFeatures` / `Clips` / `clip_paths`，
  `nuscenes_index` 多写一张 `cam_front.parquet`（每一帧 CAM_FRONT，含 sweeps），
  `probe_v0.py` 加 `--clip-batch-size`（V-JEPA 2 一个 batch 是几条 64 帧的 clip，默认 2）。
- **模型还没下完**：V-JEPA 2 只有 config，权重未到；SigLIP2 还没开始。下载由另一个 session 的队列管，
  这边不碰网络。模型一到就能跑：
  `scripts/probe_v0.sh --version v1.0-trainval --steps features --backbones siglip2,vjepa2`
  然后 `scripts/planner_v0.sh --version v1.0-trainval --clip-set vjepa2 --sets '"'"'...'"'"'`。
- 代价还没实测（V-JEPA 2 每个样本要解 64 张 JPEG，I/O 是主线的 20 倍），**跑之前先 benchmark**。
