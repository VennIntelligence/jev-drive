...
- 2026-09-26 10:48 CST（box 时钟，= 提交 48145ce 的时间；原手写 10:50）[G2] 分步估时、G1c 的定义与读法、G2 的操作化，写于 G1c 与 G2 的任何数字之前（此前只读过 G0 / G1 / G3 已发表的数与 I3 的索引结构：
...
### G1c：g₃ × 常数减速对照（2026-09-26 10:54–11:18 box 时钟，CPU；登记与读法见偏离日志 [G2] 10:48、11:00、11:22）

代码 `jevdrive/real_g2.py`（`g1c-wod` / `g1c-nav-write` / `nav-table`），run `runs/real-data-transfer/g2-g1c-*/`、`g2-nav-table/20260926-111828`；小表 [research/results/real-data-transfer/g2/](../research/results/real-data-transfer/g2/)（`g1c_*`、`navsim_paired.csv`）。
常数 c = g₃ 加权的 M-C Δ 平均（只留纵向），arm = prior + g₃ · c；g₃ 与 M-C Δ 都是 G1 / E1 存的原样。复现核对：WOD 上重算的 g₃ × M-C 与 G1 表逐项一致（Cinque −0.01 [−0.04, +0.02]），I3 上 68.4% 与 G1 相同。
NAVSIM 只打了 g₃ > 0 的 1 247 个 token（加 200 个核对 token），其余 token 取 prior 的分数：v1.1 PDMS 的核对全部逐位相等，EPDMS 不等，所以本节只报 PDMS。

常数本身（navtest，g₃ 加权）：Cinque 2 s 处 −0.22 m、4 s 处 −6.1 m；Lebowski −1.5 m / −12.5 m（纵向，负 = 更慢）。g₃ > 0.01 的 token 占 5.0% / 7.8%。

| navtest PDMS Δ（token bootstrap 95% CI） | 全部 12 146 | g₃ > 0.01 | 有接近车辆 4 726 | 走廊内行人 / cyclist 897 | 直行 4 365 |
|:--|:--|:--|:--|:--|:--|
| Cinque g₃ × M-C（G1） | +0.46 [+0.34, +0.59] | +9.22 | +1.16 | +1.53 [+0.81, +2.32] | +0.68 |
| Cinque **g₃ × 常数** | **+0.53 [+0.40, +0.67]** | +10.60 | +1.34 | +1.64 [+0.89, +2.48] | +0.87 |
| Cinque **(g₃ × M-C) − (g₃ × 常数)** | **−0.07 [−0.12, −0.02]** | −1.37 [−2.42, −0.41] | −0.18 [−0.31, −0.05] | −0.12 [−0.31, +0.08] | −0.20 [−0.34, −0.07] |
| Lebowski g₃ × M-C（G1） | +0.36 [+0.22, +0.51] | +4.66 | +0.85 | +1.55 [+0.77, +2.42] | +0.69 |
| Lebowski g₃ × 常数 | +0.42 [+0.28, +0.57] | +5.43 | +1.01 | +1.72 [+0.92, +2.60] | +0.84 |
| Lebowski (g₃ × M-C) − (g₃ × 常数) | −0.06 [−0.09, −0.03] | −0.77 [−1.19, −0.37] | −0.16 [−0.24, −0.08] | −0.17 [−0.34, −0.04] | −0.15 [−0.24, −0.07] |

2D 常数（横向也保留均值）与纵向常数在每一格上相差 ≤ 0.01。g₃ × 常数的激活率在全部 token 上为 0%。
WOD（描述，g₃ 在 rater 帧上几乎不开）：全部 rater 帧 g₃ × 常数 −0.02 [−0.05, +0.00] / −0.03 [−0.08, +0.00]，(M-C) − (常数) +0.01 [−0.00, +0.03] / +0.02 [+0.00, +0.04]，量级都在 0.05 RFS 以下。
I3（描述）：g₃ × 常数 68.5% / 69.8%，与 g₃ × M-C 的 68.4% / 70.1% 相同。

**读法（按登记）**：主读数 (g₃ × M-C) − (g₃ × 常数) 在 NAVSIM 全部 token 上两个模型的 CI 都**整体 < 0**，所以 **NAVSIM 上 g₃ × M-C 的正数由「lead 门 + 一个与场景无关的刹车」解释，与 CARLA 配对学到的逐帧方向无关**；
