...
- 2026-09-26 15:27 CST [A] 放置 null 修正的 smoke（GPU 3、单 server，3 个世界）与第二处修正（写于 45 个世界重跑之前）。25300 HazardAtSideLane：自行车全程在 d ≈ −2.6 m（路肩），expert keep ✓。
  ...
  第二处修正：位移改按 ego 路线（`RouteScenario.route`）在最近点的切向求右向量和横向偏移，车道宽取路线点所在车道；首 tick 平移与自行车 offset 的修正保留。重跑前再 smoke 这两个世界。
- 2026-09-26 15:44 CST [A] 第二处修正的 smoke 仍不对（1852 一辆事故车到对向车道、一辆掉出地图，3464 的车又回到 d = −0.78 m）：逐 tick 看是**先被挪对、随后被物理弹回**——
  开着物理的停放车 / 事故车被放到路缘或墙边，碰撞解算把它推回车道或推飞。第三处修正：被平移的静态 actor 关掉物理（它们本来就不动）。smoke（同两个世界）：
  3464 的车停在 d = −4.28 m、1852 的三辆车在 −2.92 / −2.91 / −2.96 m，都一直待在路肩，expert 两个都 keep。开跑 45 个放置 null 世界：GPU 0、1、2、3、5 各 3 个 server（共 15），每个 4 核，`runs/p6/gen`。
...
| YOLO26x-640 + DA3METRIC-LARGE（副，12:36 补） | 0.91 / 0.77 / 0.402 | 0.93 / 0.76 / 0.590 | 0.67 / 0.93 | 两个都 ≥ 0.40（不进判格） |
...
副读数 DA3METRIC-LARGE（12:36 CST 在 GPU 4 上补完 nuScenes 部分，评测 `runs/night2/n5/eval/20260926-123639`）：YOLO26x-640 检测上 P5 0.91 / 0.77 / **0.402**、nuScenes 0.93 / 0.76 / **0.590**，P5 hazard 行人全部 0.67 / ≤ 20 m 0.93；SAM 3.1 检测上 20–40 m P5 0.418 / nuScenes 0.589。深度 / 平地放置距离比（20–40 m 行人）P5 0.84、nuScenes 0.87–0.90。副读数在 P5 上刚过线、主读数刚不过（0.402 对 0.397），判格按登记以主读数为准，不改；两个独立模型数几乎相同，P5 残差不是单一模型的问题。
...
图：YOLO26x-640 检测，行人 BEV 召回按距离档；灰 = 平地，蓝 = UniDepth v2（给内参），橙 = DA3METRIC-LARGE，斜线 = 不经 BEV 的图像平面召回；虚线 0.40、点线 0.30 是 N5 的判据线。要看的是 20–40 m：nuScenes 上 UniDepth 几乎追平图像平面召回，P5 停在线下 0.003。
...
**null 与镜像题**：放置 null keep **40 / 45 = 0.889（门 ≥ 0.90，按登记仍不过）**——这是修过生成器、重跑 45 个世界之后的数（16:00 就地改；原写「35 / 45 = 0.78」，那一版生成器有三处 bug，诊断与修正见 N1 节 15:02 / 15:27 / 15:44 条）；剩下的 5 个 stop 在同 case 的 x₀₀ 里也 stop（背景交通），事后口径「放置 null 的模式 = x₀₀ 的模式」45 / 45；镜像题 stop 20 / 25 = **0.80（门 ≥ 0.80，过）**，HazardAtSideLaneTwoWays 的 5 个是跟车不停（记成 keep）。
...
放置 null 按规则修了生成器并重跑（16:00 就地改；原写「要先修并重跑」）：修完后与障碍有关的 stop 全部消失，按登记口径 0.889 仍差一个世界，差的 5 个都是与障碍无关的背景交通停车（x₀₀ 同样停），生成器已经修不动它们；考生读放置 null 时用「与 x₀₀ 同模式」的 40 个 keep 世界作分母，并注明登记门槛未过。
