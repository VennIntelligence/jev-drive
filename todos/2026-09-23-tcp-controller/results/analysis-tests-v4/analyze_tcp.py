...
def main():
    ...
    for route in ROUTES:
        ...
        quality=lambda x: bool(x['full']['rows']) and not x['quality']['parse_errors'] and not x['quality']['truth_unavailable_ticks'] and not x['quality']['nonnominal_intervals'] and not x['quality']['unexpected_arm_ticks'] and not x['quality']['source_changed_during_read'] and x['quality']['required_sensor_alignment_ticks']==x['full']['rows'] and x['full']['speed_error_mps']['n']>0 and x['full']['jerk_lon_mps3']['n']>0
    ...
    for identity,case in cases.items():
        official=case['official'] or {};scores=official.get('scores') or {};q=case['quality'];f=case['full']
        entry=dict(identity=identity,route_id=case['route_id'],arm=case['arm'],attempt_path=case['attempt_path'],harness_status=case['attempt'].get('status'),official_status=official.get('status'),completion_pct=scores.get('score_route'),driving_score=scores.get('score_composed'),
                   telemetry_frames=f['rows'],desired_speed_frames=f['speed_error_mps']['n'],truth_frames=f['rows']-q['truth_unavailable_ticks'],required_sensors_synced=q['required_sensor_alignment_ticks'],prediction_missing=q['prediction_missing_ticks'],prediction_fault=q['prediction_fault_ticks'],reverse_motion=q['reason_counts'].get('reverse_motion',0),tiny_negative_raw_speed=q['tiny_negative_raw_speed_ticks'],
                   speed_error_rms=f['speed_error_mps']['rms'],accel_lon_abs_p95=f['accel_lon_mps2']['abs_p95'],jerk_lon_abs_p95=f['jerk_lon_mps3']['abs_p95'],accel_lat_abs_p95=f['accel_lat_mps2']['abs_p95'],jerk_lat_abs_p95=f['jerk_lat_mps3']['abs_p95'],target_change_abs_p95=f['target_change_rate_mps2']['abs_p95'],low_speed_runs_ge_5s=len(case['low_speed_runs_ge_5s']),steer_saturated_ticks=f['steer_saturated_ticks'],wall_s=case['attempt'].get('wall_s'))
        for kind,events in (official.get('infractions') or {}).items():
            if isinstance(events,list):entry['infraction_'+kind]=len(events)
        compact.append(entry)
    if compact:
        with (out/'cases.csv').open('w',newline='') as stream:
            writer=csv.DictWriter(stream,fieldnames=list(dict.fromkeys(k for r in compact for k in r)));writer.writeheader();writer.writerows(compact)
    result['route_equal_arm_means']={arm:{name:stats(c['full'][metric][stat] for c in cases.values() if c['arm']==arm)['mean'] for name,metric,stat in [('speed_error_rms','speed_error_mps','rms'),('accel_lon_abs_p95','accel_lon_mps2','abs_p95'),('jerk_lon_abs_p95','jerk_lon_mps3','abs_p95')]} for arm in ARMS}
    result['route_equal_means_complete_pairing_required']=result['all_six_behavior_data_available']
