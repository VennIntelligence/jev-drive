...
def save(fig,out,name):
    fig.tight_layout();fig.savefig(out/(name+'.png'),dpi=250);fig.savefig(out/(name+'.pdf'));plt.close(fig)
...
def main():
    ...
    for route in ('24211','1711','1773'):
        ...
        fig,axs=plt.subplots(1,len(active),figsize=(7*len(active),8),squeeze=False)
        for col,(arm,case,raw) in enumerate(rawsets):
            ax=axs[0,col];valid=[r for r in raw if vector((r.get('truth') or {}).get('xyz'))];xy=[r['truth']['xyz'] for r in valid]
            if xy:ax.plot([p[0] for p in xy],[p[1] for p in xy],color='black',lw=1.5,label='Actual ego actor position')
            ...
            if navpath.exists():
                ...
                if nav:ax.plot([p[0] for p in nav],[p[1] for p in nav],color='gray',ls=':',lw=1,label='Navigation reference')
            ...
            for r in valid:
                ...
                for offset,color,style,label in [(0.,'#e66101','-','Predictions: assumed actor origin'),(-1.4,'#5e3c99','--','Predictions: assumed GNSS origin (-1.4 m)')]:
                    ...
                    ax.plot([p[0] for p in world],[p[1] for p in world],color=color,ls=style,alpha=.45,lw=.8,label=label if sampled==1 else None)
            ...
            ax.set_title(route+' / '+arm);ax.set_xlabel('CARLA world x (m)');ax.set_ylabel('CARLA world y (m, down on plot)');ax.invert_yaxis();ax.set_aspect('equal',adjustable='datalim');ax.grid(alpha=.2);ax.legend(fontsize=7,loc='best')
        fig.suptitle('Predictions sampled every 2 simulation seconds; anchoring hypotheses only',fontsize=10)
