...
def main():
    ...
    for route in ('24211','1711','1773'):
        ...
        for col,(arm,case,raw) in enumerate(rawsets):
            ...
            ax.set_title(route+' / '+arm,fontsize=9,loc='left');ax.set_xlabel('CARLA world '+('x' if horizontal==0 else 'y')+' (m)');ax.set_ylabel('world '+('x' if vertical==0 else 'y')+' (m)');ax.set_aspect('equal',adjustable='box');ax.grid(alpha=.2);ax.legend(fontsize=7,loc='upper center',bbox_to_anchor=(.5,-.65),ncol=4)
        if shown:
