...
- [2026-09-26 多榜前 10 交集](2026-09-26-top10-intersection.md)：12 张榜 / 子榜放大到前 10（补齐上次漏的 BridgeDrive、HF 榜匿名第一、ZSD-Titan、Kyber-E2E 12.455、DriveZero-Scale 等第一名），按方法族求交集：没有一个族在真实数据开环与 CARLA 闭环都稳居前 10；选 6 族（DrivoR 系、WA-JEPA、TransFuser-LEAD、SimLingo、SparseDrive、ZTRS 对照），新装 DrivoR / WA-JEPA / SparseDriveV2 / ZTRS / BridgeDrive / BLUE 并 smoke；P5 v1、I3、WOD、NAVSIM、nuScenes 的考试预登记与约 17 GPU·h 估计，未跑。
- [2026-09-26 公开 state-space policy 的行为考卷](2026-09-26-state-space-policies.md)：BehaviorBench 的 PPO / conditioned PPO 在 WOMD interactive val 上的预登记反事实考卷（删对手、插静止车、平移起点）；PPO 对对手的让行与横向反应是 IDM 的 3 倍，只有 conditioned 的激进系数干净地绕，没有一个会回线；附 PufferDrive 路边停车变成隐形幽灵的仿真 bug（[清单](../research/state-space-policies.md)）。
- [2026-09-26-night-queue-3.md](2026-09-26-night-queue-3.md) — 夜间队列 3（重排版）：P6 上读全部考生（Q1）、在 openpilot 冻结特征上用配对差分激发绕行（Q2）、P6 v1 扩容与 recovery 题（Q3）、真实非 hazard 帧上的 Δ 零约束（Q4）、六族 hack 核查（Q5）、主表口径（Q6）、Bench2Drive 220 闭环（P7 执行层，含绕行 head 与 desire 执行器）；四条 lane 的卡 / 核 / 时间表与一次性链式脚本规则
- [2026-09-26-night-queue-4.md](2026-09-26-night-queue-4.md) — 队列 4：背题的签名（闭环 ghost test：删掉 scenario 后是否仍在原位置减速 / 绕行；触发点平移与换类的扰动崩塌；BLUE − SimLingo 闭环复核第 38 条）、材料包阶梯（开环 NAVSIM + 闭环 B2D 两折交叉拟合，分数动、能力不动；背题溢价作阳性对照）、B2D 训练集与 220 路线重叠核查、JEPA ⊕ openpilot 世界模型配对测试
