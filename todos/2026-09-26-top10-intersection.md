...
- 2026-09-26 15:45 CST [T3] **补 5.5 的 BLUE vs SimLingo 一格（main 15:40），写于任何 SimLingo 数字之前**：
  ...
  2. **agent 原样**：SimLingo 仓库 `743b243` 自己的 `team_code/agent_simlingo.py`（不是 BLUE 仓库里改过的副本），`envs/simlingo`，config 与 BLUE 的 `config_simlingo.py` 逐字相同（单前视 1024×512 FOV 110、`eval_route_as=target_point`、`use_cot=True`，所以每帧都先生成语言再出动作）；
     global plan 经 SimLingo 自带 Bench2Drive 副本的基类 `set_global_plan`（每 50 m 下采样，与 BLUE 实际走的相同）。
  3. **输入与适配**：与 BLUE 完全相同——同一批重录的 rgb_0 PNG、GNSS / IMU / 速度，同一个离线 runner（`scripts/top10_t3_blue.py --model simlingo`），UKF 控制输入喂 expert 上一 tick 的控制、模型只在被引用的相机 tick 跑、其余 tick 喂 64×32 黑图、batch 1。
     跑之前做同 BLUE 的 E3 检查（1 个世界：捷径对作者每 tick `run_step`，逐位相同才开跑）。
