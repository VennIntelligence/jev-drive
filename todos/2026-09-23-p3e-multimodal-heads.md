...
状态: done（2026-09-24；结论待 lead 写进 decisions）
主题: ../research/prediag-2026-09/README.md

## 目标

P3(d″) 量出来 Qwen3-VL-4B 原生视频输入（`qwenvid_p3`）是整条阶梯里唯一两个方向 CI 都不跨零的表征（第 24 条；下面的 2B 插入显示同一视频通路的 2B `L14_last` 也满足，所以「唯一」指的是这条视频通路），
...
- [x] 决定性检查 (a)：`trainfit`，train 训、val 评的 d″ ridge_late（第 22 条口径；全量 263 shard，2026-09-24 05:00）
- [x] (b)：多模态 head 在 train 特征上重跑（含 pca16 与空间 token 条件的 diffusion）
...
所以没有「只有 L21 满足」要交给 lead 的情况。一句话：2B 在方向 1 上和 4B 一样好，方向 0 上弱一截，**d″ 仍是这个量上最强的表征**（*2026-09-23 修正*：原来写「d″ 仍是唯一两个方向 CI 都不跨零的表征」，与上表矛盾：2B `L14_last` 的 CI 两个方向也不跨零，[−0.042, −0.006] / [−0.064, −0.006]，只是点估计小一截）；
...
### Stage A3：空间 token 条件更差（2026-09-23 18:05）

run：`$DATA_DIR/runs/waymo_heads/p3e-a3/20260923-175341/`，小表 `research/results/p3e-heads/a3/`。

| 方向 | arm | pre-onset Δ vs `ridge ego` | 视觉增量 vs `diff ego`（pre-onset）| DiD | RFS Δ | inner top-1 |
|--:|:--|:--|:--|--:|--:|--:|
| 0 | `diff ego M20` | +0.598 [+0.404, +0.813] | — | +0.396 | +0.297 | 1.94 |
| 0 | `diff qwenvid L18_grid 4x4` | +1.836 [+1.291, +2.569] | +1.290 [+0.799, +1.935] | +0.517 | +0.006 | 2.88 |
| 1 | `diff ego M20` | +0.610 [+0.367, +0.878] | — | +0.450 | +0.205 | 2.09 |
| 1 | `diff qwenvid L18_grid 4x4` | +1.635 [+1.133, +2.247] | +1.070 [+0.612, +1.672] | +0.664 | −0.178 | 3.02 |
...
### (a) 初步，第二次：加上 thin 倍权重（2026-09-23 21:30）

run：`$DATA_DIR/runs/waymo_qwenvid/trainfit/20260923-212924/`（此时 68 个 train shard、34 930 行），小表 `research/results/p3-qwenvid-trainfit/prelim-68shards/`。
加权版：被稀疏的帧（非 pre-onset、非转弯）权重 4，其余 1，`ridge ego` base 同样加权，λ 用加权的 grouped CV 选（`waymo_l0.wridge_cv`）。

| arm | 均匀：pre-onset Δ [CI] | 均匀：RFS Δ [CI] | 加权：pre-onset Δ [CI] | 加权：straight Δ | 加权：DiD | 加权：RFS Δ [CI] |
|:--|:--|:--|:--|--:|--:|:--|
| A pooled | −0.033 [−0.077, +0.013] | −0.096 [−0.197, −0.001] | −0.020 [−0.064, +0.025] | −0.105 | +0.085 | −0.032 [−0.114, +0.047] |
| d″ `L18_last` | −0.033 [−0.078, +0.017] | −0.387 [−0.517, −0.252] | −0.036 [−0.080, +0.015] | −0.068 | +0.032 | −0.188 [−0.297, −0.078] |
| d″ `L18_mean` | −0.050 [−0.098, −0.002] | −0.416 [−0.555, −0.284] | −0.047 [−0.101, +0.003] | −0.104 | +0.057 | −0.241 [−0.354, −0.135] |

读法：
- **构成解释了 RFS 掉分的一半左右，不是全部**：加权把 A 的 RFS Δ 拉回 −0.03（CI 跨零，与 P0 的 −0.087 同一量级），
  d″ 从 −0.39 / −0.42 回到 −0.19 / −0.24，但 CI 仍不跨零。也就是说在 train 训之后，视频特征在 rater 的口径上**比单帧特征更差**，这是和 ADE 口径相反的方向。
- pre-onset 上加权几乎不动 d″（−0.036 / −0.047），`L18_mean` 的 CI 上界从 −0.002 变成 +0.003——正好压在门槛线上，全量 shard 到齐前不下结论。
- 同一套数据只多了 2 个 shard（34 433 → 34 930 行），A 的均匀版 pre-onset 就从 −0.012 变到 −0.033：arm A 在这个量上本身抖 2 cm，d″ 两次几乎不变（−0.051 / −0.050）。

### 决定性检查 (a)：全量 train 训、val 评（2026-09-24 05:00）

抽取完成：356 个 shard（93 val + 263 train），157 196 行，39 GB（含 4×4 空间摘要），墙钟 13.8 h（与 vlm / p4 共享卡，单独时 183–220 ms/帧）。
run：`$DATA_DIR/runs/waymo_qwenvid/trainfit/20260924-050013/`，小表 `research/results/p3-qwenvid-trainfit/full/`。
fit 137 533 个 train 行（thin=4），eval 同一个 P3 val 子集（pre-onset 第 1–9 档 1281 帧、rater 478 帧），单方向，第 22 条口径。

Δ = arm − `ridge ego`：

| arm | 训练权重 | pre-onset Δ（第 1–9 档）[CI] | pre-onset（全部档，侧栏）| straight Δ | DiD | RFS Δ [CI] | 第 10 档 RFS Δ | 第 10 档 vs rater_best |
|:--|:--|:--|:--|--:|--:|:--|--:|--:|
| A pooled（单帧 4B） | 均匀 | −0.012 [−0.061, +0.046] | −0.024 | −0.096 | +0.085 | −0.225 [−0.355, −0.104] | −0.107 | −0.121 |
| d″ `L18_last` | 均匀 | −0.019 [−0.066, +0.030] | −0.052 | −0.040 | +0.021 | −0.492 [−0.630, −0.343] | −0.617 | −0.212 |
| d″ `L18_mean` | 均匀 | **−0.062 [−0.111, −0.015]** | −0.092 | −0.095 | +0.033 | −0.421 [−0.563, −0.306] | −0.500 | −0.210 |
| A pooled | thin 加权 | −0.001 [−0.054, +0.052] | −0.014 | −0.109 | +0.108 | −0.076 [−0.173, +0.016] | −0.039 | −0.122 |
| d″ `L18_last` | thin 加权 | −0.022 [−0.071, +0.029] | −0.054 | −0.071 | +0.049 | −0.246 [−0.365, −0.130] | −0.407 | −0.161 |
| d″ `L18_mean` | thin 加权 | **−0.058 [−0.118, −0.001]** | −0.092 | −0.102 | +0.044 | −0.311 [−0.431, −0.201] | −0.312 | −0.203 |

d″ 对单帧 arm A 的直接配对（同一批帧，`scripts` 外的一次性计算，逻辑同 `waymo_p1.paired`）：

| tap | 训练权重 | pre-onset（第 1–9 档）d″ − A | straight d″ − A | RFS d″ − A |
|:--|:--|:--|:--|:--|
| `L18_mean` | 均匀 | **−0.051 [−0.092, −0.011]** | +0.001 [−0.012, +0.014] | −0.196 [−0.298, −0.099] |
| `L18_mean` | 加权 | **−0.057 [−0.097, −0.016]** | +0.007 [−0.007, +0.021] | −0.235 [−0.330, −0.155] |
| `L18_last` | 均匀 | −0.007 [−0.056, +0.040] | +0.057 [+0.040, +0.074] | −0.267 [−0.392, −0.133] |
| `L18_last` | 加权 | −0.021 [−0.069, +0.025] | +0.038 [+0.023, +0.054] | −0.169 [−0.272, −0.067] |

读法：
- **ADE 口径上 d″ 活下来了，而且比半 val 更干净**：`L18_mean` 在 train 训之后 pre-onset Δ = −0.062（均匀）/ −0.058（加权），CI 都不跨零，
  按第 20 条的 −0.05 m 门槛单方向过线；对单帧 arm A 的配对差是 −0.051 / −0.057，CI 不跨零，**而 straight 上两者一样（+0.001 / +0.007）**——
  时间输入买到的东西正好集中在 pre-onset 上，这是 d‴ 里 V-JEPA 没做到的（它缩到 −0.030 且跨零）。`L18_last` 在 train 训后不站：−0.019 / −0.022，跨零。
  半 val 上两个 tap 各在一个方向过线，这里 mean 这个 tap 在更大的 fit 上站住、last 没有。
  相对 `ridge ego` 的 DiD 仍为正（+0.03 / +0.04，跨零）：d″ 在 straight 上也有增益，所以「增量集中在先验最弱处」只在对 A 的比较里成立，不在对 ego 的比较里。
- **RFS 口径上 d″ 显著更差**：相对 `ridge ego` −0.42（均匀）/ −0.31（加权），相对 A −0.20 / −0.24，CI 都不跨零；第 10 档掉得最多（−0.50 / −0.31）。
  加权把 A 的 RFS 拉回 P0 的量级（−0.08，跨零），但 d″ 仍掉 0.3。换句话说，d″ 的回归预测更贴 logged future（ADE），却更常落到 rater trust region 之外——
  第 22 条要并排报 RFS 正是为了这种情形。推测（未验证）：视频特征让 ridge 更敢于预测「减速 / 停」（log 在第 10 档正是这种），而 rater 的 proposal 继续走，
  这与第 20 条修正框里「第 10 档是 log 比 proposal 更早更重的减速」一致；可以按第 10 档里预测行程长度与 log / rater_best 的差来验证。
- 判定：按预写门槛，d″ `L18_mean` 在 ADE 主判上**确认**（单方向、train 训），`L18_last` 不确认；但 RFS 反向且显著，
  这一条要交给 lead 决定怎么进 decisions（我不改 decisions.md）。

### (b) 多模态 head 在 train 特征上（2026-09-24 07:20）

run：`$DATA_DIR/runs/waymo_heads/p3e-train/20260924-050109/`，小表 `research/results/p3e-heads/train/`。
与 (a) 同一批行：fit 137 533 个 train 行（thin=4，均匀权重），eval 是 P3 val 子集。词表与 Stage A 相同（train 全部 logged future 上的 k-means）。

| arm | pre-onset Δ vs `ridge ego` [CI] | straight Δ | DiD | RFS Δ vs `ridge ego` [CI] | 第 10 档 vs rater_best | top-1 − `ridge_late`（同 tap）pre-onset | RFS − `ridge_late`（同 tap）|
|:--|:--|--:|--:|:--|--:|:--|:--|
| `ridge_late L18_last`（= (a)） | −0.019 [−0.066, +0.030] | −0.040 | +0.021 | −0.492 [−0.630, −0.343] | −0.212 | — | — |
| `ridge_late L18_mean`（= (a)） | −0.062 [−0.111, −0.015] | −0.095 | +0.033 | −0.421 [−0.563, −0.306] | −0.210 | — | — |
| `cls ego K1024` | +0.418 [+0.261, +0.576] | +0.443 | −0.025 | +0.178 [−0.024, +0.389] | −0.234 | — | — |
| `cls_late L18_last` | +0.403 [+0.245, +0.550] | +0.442 | −0.038 | +0.174 [−0.034, +0.381] | +0.491 | +0.423 [+0.276, +0.564] | +0.666 [+0.435, +0.881] |
| `cls_late L18_mean` | +0.380 [+0.218, +0.537] | +0.459 | −0.080 | +0.226 [+0.040, +0.439] | +0.403 | +0.442 [+0.292, +0.595] | +0.647 [+0.443, +0.880] |
| `diff ego M20` | +0.210 [+0.068, +0.351] | −0.042 | +0.252 | **+0.356 [+0.172, +0.561]** | −0.387 | — | — |
| `diff L18_last` | +0.248 [+0.086, +0.388] | +0.157 | +0.091 | +0.474 [+0.259, +0.679] | −0.987 | +0.267 [+0.104, +0.407] | +0.966 [+0.743, +1.171] |
| `diff L18_mean` | +0.165 [+0.014, +0.317] | +0.215 | −0.050 | **+0.488 [+0.277, +0.694]** | **−1.747** | +0.227 [+0.080, +0.372] | +0.909 [+0.685, +1.137] |
| `diff L18_last pca16` | +0.324 [+0.176, +0.460] | +0.032 | +0.291 | +0.402 [+0.213, +0.610] | −0.630 | +0.343 [+0.184, +0.481] | +0.894 [+0.704, +1.104] |
| `diff L18_mean pca16` | +0.137 [+0.007, +0.267] | +0.014 | +0.123 | +0.464 [+0.257, +0.677] | −0.967 | +0.199 [+0.073, +0.333] | +0.885 [+0.671, +1.117] |
| `diff L18_grid 4x4` | +0.282 [+0.115, +0.442] | +0.165 | +0.117 | +0.360 [+0.135, +0.578] | −1.545 | +0.301 / +0.345 | +0.852 / +0.781 |

家族内部的视觉增量（vision arm − 同家族 ego arm）：pre-onset ADE 上 cls +0.005 / +0.024、diffusion −0.010 到 +0.135，CI 除 `L18_last pca16`（+0.133）外全跨零；
RFS 上（按 rater 帧配对）cls −0.005 / +0.048，diffusion +0.004（grid）到 +0.132（`L18_mean`），CI 全跨零。

诊断：分类头 top-1 ADE 2.25（Stage A 是 3.2），minADE@20 0.75，rater 帧无 mode 入 trust region 4.2–4.6%，词表缺口 0.2%，
pre-onset / straight oracle 比 2.0；diffusion top-1 1.85–2.00，20 个 mode 的 oracle 0.96–1.05（原始 anchor 1.83），rater 缺口 10–11%。

**按预写的表读**：两个家族的 top-1 仍比同 tap 的 `ridge_late` 在 pre-onset ADE 上差（+0.20 到 +0.44，CI 不跨零），同家族的 ego-only 对照也差同样量级
（cls +0.42、diffusion +0.21），落**第三行（数据 / 头本身的下界，不在特征）**；和半 val 比，差距从 1 m 缩到 0.2–0.4 m，diffusion 里特征不再伤 head（A2 / A3 的第二行不复现）。
**但 RFS 完全反过来**：ridge 在 train 训后 RFS 掉 0.4–0.5，而两个多模态家族都**赢** `ridge ego`，diffusion 最多（+0.36 到 +0.49，CI 全不跨零），
相对同 tap 的 `ridge_late` 高 0.65–0.97。第 10 档对 rater_best 的 ADE，`diff L18_mean` 降 1.75 m，是整个项目里最大的数。
这就是第 10 条说的「分类 / 多模态输 ADE、赢 trust region」，第一次在 Waymo 的真 RFS 上、以 train 规模的数据看到，而且量级很大。
视觉在多模态 head 里的增量（RFS +0.05 到 +0.13）方向为正但 CI 跨零：RFS 上的大头来自 head 的形式（多模态 + 打分），不是来自视频特征。
这条要交 lead：它改写的是「用什么 head 读」而不是「用什么表征」。
