...
### D1. smoke 结果（2026-09-25 02:22，4/5 个阶段；`shadow-cinque` 没跑，见下）

| 路线 | 预注册 smoke（修复前） | fixed-lebowski（同一控制器 + 修复） | zoo-lebowski | zoo-cinque | native-cinque |
|---|---|---|---|---|---|
| 2390 | 2.1 | 0.0（没动） | 0.0（没动） | 0.0（没动） | 0.0（没动） |
| 24211 | 3.0 | 18.0（RC 100） | 18.0（RC 100） | **59.1**（RC 100） | 0.0（没动） |
| 1711 | 2.5 | 0.0（没动） | **100.0**（RC 100，35 s 后起步） | 0.0（没动） | 0.0（没动） |
| 2373 | 6.0 | 0.0（没动） | **50.7**（RC 78.6，45 s 后起步） | 20.7（RC 52.5） | 0.0（没动） |
| 3564 | 0.0 | 0.0（没动） | 0.0（没动） | 0.0（没动） | 0.0（没动） |
| **均值 DS** | **2.7** | **3.6** | **33.7** | **15.9** | **0.0** |

DS 是官方 `score_composed`，n = 5、单 seed。“没动”指整条路线车速从未超过 0.5 m/s，最后被判 blocked（RC 0）。

读法：修复之后**不再有冲出车道**。车一旦动起来，openpilot 能把路线开完（24211 在三个配置里 RC 100，1711 在
Lebowski + Zoo PID 下 DS 100）。新的主导失败是**从静止起步**：20 个路线 × 配置里有 12 个车从头到尾没动过，plan 在 5 s
处的距离中位数只有 0.1–1.3 m，也就是模型自己一直在说“停着”，控制器照做了。这与 WOD-E2E 上的发现一致（起始车速
< 0.5 m/s 的帧上 zero-shot 模型不比“继续停着”好），也符合 openpilot 在真车上的用法：多数车型在完全停下后要驾驶员按
resume 或踩油门才重新起步，模型从来不需要自己决定“从静止出发”。native 控制器 5/5 没动，是因为它的起步条件（desired accel
> 0.2 m/s²）比 Zoo PID（plan 平均速度 > 0.4 m/s）更严；这个阈值是预先定的，没有调。
...
### D4. 适配验收结果（2026-09-25 11:16，slot `op-b2d-accept`，不计分）

Lebowski + TCP 伙伴，5 条验收路线，Zoo PID 两组开关各一遍；`scripts/zeroshot_b2d_op_accept.py` 的输出在 box 的
`runs/zeroshot-exam/b2d-op/accept.json`。

| 判据 | f1（每次规划调一次） | f1f2b（每 tick 调一次） |
|---|---|---|
| A1 基础设施、A2 参考点、A3 时序、A4 预热、A6 车道、A8 换人期间的红灯 / 停车标志、A11 控制器一致 | 通过 | 通过 |
| A5 静止起步 | 4/5 条 1 s 内起步；3144 起点就是红灯，TCP 刹停 45 s、绿灯后起步 → **已解释** | 同左 |
| A7 路口左右转 | 4/4 通过 | 3/4；2086 左转时 TCP 在路口内与车流碰撞 3 次后被卡住 → **已解释（伙伴的驾驶结果）** |
| A9 停后再起步 | 3540 在两次碰撞（模型在车队里 0.5 m/s 追尾，随后 TCP）后被顶住 60 s → **已解释** | 2086、3540 同样是碰撞后被顶住 |
| A10 plan 坐标系 / 朝向 | 2 s 横向误差中位数 0.21 m（n = 12）；左右 / 朝向符号的合格样本 < 5，不判 | 0.09 m（n = 14），同左 |
| 平均 DS / RC | 70.8 / 84.2 | 62.5 / 73.9 |
| 模型开的距离 / 时间占比 | 28% / 8% | 14% / 3% |

没通过的三项都是驾驶结果（红灯前等待、碰撞后被卡住、TCP 在路口里的碰撞），不是适配错误；按清单“失败必须由驾驶者
自己的输出解释”的规则写成 `accept-waiver.json`，smoke3 只在冻结那组的每个失败项都有书面解释时才开始。A10 在这 5 条
路线上几乎没有检验力（模型很少在 2 m/s 以上开车），smoke3 的 5 条常规路线会补上。

**验收里最重要的发现不是哪一项失败，而是模型开得太少。** TCP 出厂的低速油门上限让它只开到 1.4–1.5 m/s；在这个速度上
openpilot 的 plan 在 1 s 处只走 ~1.2 m，Zoo PID 判“实际速度 > 1.1 × plan 速度”而刹车，于是“模型未就绪”一直成立，
TCP 以 1.4 m/s 继续开（2903 上连续 22 s）；路口区（前 15 m 到后 5 m）里 TCP 也是这个速度。这 5 条是以路口为主的短路线，
smoke3 的 9 条里有 5 条常规路线，占比会高一些；E4（模型开的距离 ≥ 50%）会决定 Zoo PID 这组能不能进全量，不满足时
依次看 native（openpilot 自己的 desired accel 在同样的 1.4 m/s 上是 +1.1 ~ +1.5 m/s²，会接手），都不满足就不跑全量、
回来找用户。

**耗时（全量之前的 profiling）。** 4 个 worker 并行时每 tick 0.42–0.45 s（验收），其中 agent 自己：模型开车的 tick
~11 ms（Lebowski 每 4 帧一次规划，平均 ~20 ms / tick）；TCP 有权重的 tick 另加 ~175 ms，出厂预处理占大头（三张 1600×900 的
JPEG q20 往返 ~37 ms、CPU 上缩放 ~11–22 ms）、远程前向 ~30 ms、其余是取数；剩下 ~120 ms 是等 CARLA 把 5 台相机（其中 3 台
