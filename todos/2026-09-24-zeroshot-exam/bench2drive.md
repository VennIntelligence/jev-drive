...
换成 Bench2Drive(-Zoo) 基线用的官方 PID。两个模型（Alpamayo、openpilot）用**同一份实现**：Bench2DriveZoo
branch `uniad/vad`（`498c1f7`）的 `team_code/pid_controller.py`，逐字 vendored 在 `scripts/b2d_zoo_pid.py`（OP-B2D
维护，不改）；target point 用同一 checkout 的 `team_code/planner.py` 里的 `RoutePlanner(4.0, 50.0)`（两个 branch
上除空白外完全相同），直接从 `$DATA_DIR/third_party/Bench2DriveZoo` import。我们只加一个薄 wrapper
`scripts/b2d_zoo_pid_wrap.py`（坐标转换 + Zoo agent 自带的截断），agent 里 `"controller": "zoo_pid"` 走它。
参数全用 shipped 默认值：转向 PID 0.75/0.75/0.3、窗口 40，速度 PID 5/0.5/1、窗口 40，aim point 取中点离 ego 最接近
4 m 的那段，目标速度 = 相邻 waypoint 平均间距 × 2，desired speed < 0.4 m/s 或实际速度 > 1.1 倍时刹车，
clip_delta 0.25，油门 ≤ 0.75；当 route target 比预测的 aim 更直、或预测末段方向突变且 target 近（< 10 m）时改用 target 转向。

**与其他 shipped 版本的核对。** 先写过一版直接 import AD-MLP 的 `ADMLP.model.ADMLP.control_pid`（branch `tcp/admlp`
`8a08b07`，需要 torch），逐项对过：增益、窗口、`PIDController` 的窗口均值积分与差分、aim/target 选择规则、速度与刹车
逻辑完全相同，唯一差别是 AD-MLP 版在函数里先把 (前, 右) 交换成 (右, 前)、输入是张量；TCP 的 `control_pid` 与 AD-MLP
版逐行相同（只差张量取值）。所以三者是同一个控制器；按 main 的决定只保留 UniAD/VAD 这一份，AD-MLP 版的 wrapper 已删。

**怎么接（wrapper 与 shipped agent 的差别，逐条）：**

1. 轨迹：Alpamayo 的路径在 0.5、1.0 … 3.0 s 取 6 个点（UniAD / VAD 的输出格式），以 (右, 前) 交给 `control_pid`
   ——它的 `angle = 90° − atan2(前, 右)` 只在这个坐标下成立。target 同样是 (右, 前)。
2. 调用节奏：每次规划调一次 `control_pid`，控制量保持到下一次规划。这是 AD-MLP agent 的节奏（2 Hz 推理、中间保持）；
   UniAD/VAD 每 tick 推理一次、每 tick 调用。Alpamayo 是 2 Hz 重规划，所以取 AD-MLP 的节奏。第一条轨迹到来前刹车保持。
3. target 的几何：Zoo agent 用 fsolve 求一个 Mercator 参考把 GNSS 换成 CARLA 世界坐标；wrapper 用稠密路线的精确
   GPS/world 拟合（`GPSProjector`，残差 < 1 mm）换算，planner 在世界坐标上跑，同一个点，少一个近似。每 tick 推进，
   与 Zoo agent 一样；compass 为 NaN 时置 0 也照搬。
4. agent 层照搬：steer/throttle/brake 截断，brake > 0 置 1 且油门清零。**没有照搬**：Zoo agent 在 `control_pid` 之后另加
   的低速限油门（AD-MLP 版：转向时车速 > 2.5 m/s、否则 > 3 m/s 就把油门压到 0.05，注释写的是 "avoid stuck / avoid pass
   stop, red light, collision"）。它是基线自己的驾驶启发式，不属于轨迹到控制的映射，而且会把任何 planner 的车速钉在约
   3 m/s。要带限速复现，改 wrapper 一处即可。

**其他全部不变**：rig、nav 文本、2 Hz 重规划、推理配置、seed、5 条 smoke 路线、TM seed 0，route 子进程仍在
`envs/carla`（控制器只要 NumPy）。旧控制器下的 13/220 全量结果作废，不并入。
