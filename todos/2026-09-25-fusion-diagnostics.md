...
状态: 全部完成（第 0–2 阶段 2026-09-25 20:50；第 3 阶段 P5 v1 复跑 2026-09-26 00:47）。预登记写于 18:00，任何新拟合、新抽取之前；18:06 分到 GPU 3，19:03 起 GPU 0 作第二张卡（排程 B），SAM 批量另借 GPU 1 / 4（main 批准）
...
## 第 3 阶段：P5 v1 上的复跑（2026-09-25 23:28 – 09-26 00:47）

按 reactivity 偏离 7：PDM-Lite 集（`carla_p5v1_pdm`，303 对，783 个合并 reactive 帧 / 48 条路线，行人 309 帧）与 BehaviorAgent 集（`carla_p5v1_ba`，1 081 帧 / 49 条路线，行人 406 帧）分别跑、分别报，**判据用 PDM-Lite 集**。
偏离条目（[Q1-v1]、[Q6-v1]、[Q9a-v1]、[Q9b-v1]）都在对应数字之前提交。表在 `research/results/fusion-diagnostics/{q1-v1,q6-v1,q9a-v1,q9b-v1}/`。GPU 约 2.4 GPU·h（Q9b 网格抽取为主）。

| 读数 | v0（BA，25 条路线） | v1 PDM-Lite（主） | v1 BehaviorAgent（并排） |
|:--|:--|:--|:--|
| openpilot Cinque `ridge_late` 合并翻转 | 46.7% | **0.5%** | 48.2% |
| Q1 concat − best，合并 | −19.6 / −24.5 pp | −1.9 [−5.1, +0.5] / −1.7 [−3.7, −0.1] pp | −8.1 / −11.8 pp |
| Q1 concat − best，行人 | 0 / 0 | +0.6 [0, +2.3] / 0 | −2.5 / −2.0 pp |
| Q9a Qwen pooled 配对差分，行人翻转（null） | 0%（5.3%） | 0.6% [0, 2.2]（5.0%） | **42.1% [33.5, 50.0]**（4.9%） |
| Q9b Qwen 网格 + attention，行人翻转（null） | 0%（5.7%） | 4.5% [0, 12.5]（5.1%） | **46.1% [37.7, 54.2]**（5.2%） |
| Q9b 训练对行人拟合斜率（早停 / 40 epoch） | 0.09 / 0.8 | 0.004 / −0.04 | 0.62 / 0.71 |
| Q6 GT 规则门 `gate any`（null） | 42.9%（0%） | **4.2% [1.9, 7.0]**（0%） | 50.7% [38.9, 62.3]（0%） |

（Q9b hard-example 对照：PDM 0%、BA 3.4%；pooled 线性：PDM 0.6%、BA 16.0%；Lebowski 同向。Q9b BA 的 cut-in 对 prior −9.5 pp，M-C 的 cut-in 附加条件差一点不过。Q6-SAM 不在 v1 上跑，见日志。）

**读法**。
1. **两个 expert 给出相反的答案**。BehaviorAgent 的 v1（101 条路线）上，v0 的「0」都翻过来：Qwen pooled 配对差分行人 42%、网格 46%，openpilot 合并 48%，GT 规则门 51%。v0 行人读出为 0 是数据量问题（v0 固定 40 epoch 的敏感性已有提示）。
2. PDM-Lite 集上所有考生贴地板：openpilot 0.5%、TFv6 waypoint 12.5%、GT 几何规则 4.2%、Qwen 网格 4.5%，网格 head 连训练对上的行人都拟合不了（斜率 0.004）。
   机制（描述）：PDM-Lite 在因素可见后约 0.4 s 就反应，reactive 帧里 hazard 还远 / 小、几何上还没进走廊（逐对看过一个 HighwayCutIn：反应时 cut-in 车还在侧向 10 m 外）；openpilot prior 在 reactive 帧上的 |Δ_model| 中位 0.10 m/s（BA 1.0）。
   **PDM 考卷问的是「从几乎看不见的因素上预判」，不是「对看得见的危险做反应」。**
3. **按预登记判据（以 PDM 为准）**：Q1 P5 行不变（冗余）；Q9 仍为 0（Q9a、Q9b 都不过）；Q6「GT 规则 floor ≥ 80%」不成立（4.2%）→ 反应不主要是几何的。
   BA 集把 Q9 推向另一格（「是读出问题 → Qwen 保留为行人通道」），这是同一层读数在两个 expert 上的冲突；判格按 PDM 写，冲突记在这里。
   另：decisions 第 43 条已由 main 在 v1 出数后就地修正（BA 集上 M-C 双流 43%、均匀 imitation 下的「冗余」只是训练信号的性质，撤回「行人走 SAM 通道」）；本节的 PDM 读数与之不冲突，补在该条后面。

**架构页的修订（2026-09-26）**：上面「架构方向」一页写于 v0。v1 之后：「行人走 SAM 通道」撤回（依据「Qwen 读出为 0」只在 PDM 上成立，而 PDM 上连特权几何规则也只有 4%，缺的不是感知通道而是预判能力）；
「冗余、去掉 Qwen」只在均匀 imitation 下成立，Qwen 作为配对差分训练的行人反应通道在 BA 上有用；Q6 的 decision head 动机从「几何够不够」变成「预判」。用哪种标签当主考卷（反应型 BA 还是预判型 PDM）是下一步要定的事。
