...
- **提速**（19:10，wave 1 跑到 65 / 88 条）：box 的 CPU 被占满（load 22–30），一条路线从 2.5 min 涨到 3–6 min。
  已完成的 61 条路线里，横向 pre-onset 候选 181 个全部出现在前 40 s、in-turn 94% 在前 20 s，40 s 以后几乎只剩停车排队，
  所以其余路线把录制上限从 150 s 降到 45 s、「停住多久算卡住」从 30 s 降到 20 s（`agent_config.json`）。只影响每条路线录多长，不影响标签定义。
- **加速度单位的 bug（22:50 发现，已修，索引和特征重做）**：第一次完整分析里 Waymo fit 的 `ridge ego` 在 CARLA 上 ADE 160 m（CTRV 只有 5.7 m），
  查出来是 ego 输入的加速度单位不一致：WOD-E2E 的 `accel_x / accel_y` 对 0.25 s 速度差回归的斜率是 0.243，
  **它是每 0.25 s 步长的速度变化，不是 m/s²**；CARLA 这边按 m/s² 算，量级大 4 倍，再加上碰撞时的尖峰，标准化后到 100 多个 σ，线性 ego head 外推爆掉。
  改成与 Waymo 同一个量（0.25 s 后向速度差）。分层表里「开始刹车」的「当前纵向加速度 ≥ −1 m/s²」原来直接拿 `accel_x` 比，
  对 Waymo 实际是 ≥ −4 m/s²，同样改成先除以 0.25 s。这两处改动都会改变哪些帧进层，所以按同一规则重新抽帧、重新抽特征；
  第一次分析的结果作废，不进结果表（Q1 的 AUC 不依赖 ego 输入，但抽出来的帧变了，一并重跑）。
  修完单位后 `ridge ego` 在 CARLA 上仍有 18 m：还有两处约定不一致。WOD-E2E 里是 t−0.25 s 那一格抄 t0（它的 t0 横向速度 std 只有 0.018 m/s），
  原来抄反了；速度改成后轴位置 ±1 tick（50 ms）的中心差分，和 Waymo 的「瞬时速度、0.25 s 平均等于位置差」一致，
  不再用仿真器带抖动和侧滑的刚体速度。即便如此，线性 ego head 在 Waymo 上几乎不变的维度上权重很大，
  仿真的侧滑和碰撞尖峰仍能把它推到几百个 σ 外，所以 Q3 **加报一组「ego 输入裁剪到 Waymo fit 半 0.5–99.5% 范围」的行**
  （无标签的防外推，裁剪后的 base 上再 fit `ridge_late`），与原始输入那组并排；判据两组都读，裁剪那组算「加自适应」。
