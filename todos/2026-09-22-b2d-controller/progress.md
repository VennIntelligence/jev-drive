...
正式60条/63次attempt与独立坡道hold全部结束，CARLA进程已停止。49/60完成；候选16/20对参考17/20，
全程横向RMS高10.84%，不通过原默认替代条件。三次rc139均保留并重试，11次TickRuntime不删去。
完整G4审计、对照图、37.46min成本表与904文件原始索引均已生成，见[最终报告](final-report.md)。
失败诊断166个中间文件迁至/data/runs/b2d/controller/formal-v4-failure-audit，迁移前后哈希一致，历史路径用忽略的symlink保留。

补充精确max候选G1的16/16在正式启动后完成，时间顺序明确记录；拟议的10条G2 seed1复跑没有执行，
候选已失败，不追加条件性的v4保留集。没有新真实TCP实验，已有计划按纵向两组×三路线限定下一阶段。
本阶段文档与904个正式原始文件哈希已复核，本地提交收尾；GitHub推送仍待用户稍后登录，不重复请求或尝试。
