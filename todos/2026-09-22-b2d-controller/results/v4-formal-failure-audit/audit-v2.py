...
def snapshot(root, output, report):
    ...
    for group, route_id, attempt in signature:
        ...
        item = dict(group=group, preset=group.split('-seed')[0], seed=int(group.split('-seed')[1]), route_id=route_id, attempt=attempt,
                    ...
                    invalid_pose_ticks=len(invalid), controller_invalid_motion_ticks=sum(row.get('reason') == 'invalid_motion' for _, row in control), invalid_control_ticks=q['invalid_controls'], stale_ticks=telemetry['stale_ticks'],
