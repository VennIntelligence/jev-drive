...
第一轮已完成车辆标定、G1、原四条G2、smoke、Dev10三preset两seed共60条、六条保留集三preset共18条及实坡停车。
详细结果见[文章素材](article-notes.md)。旧预算与Setup表保留为开跑前记录，当前运行版本与来源以各run的snapshot为准。
没有合格的新默认：pursuit在两seed的Dev10横向路线均值都未优于CARLA，且平均completion略低于TCP。
补充真实S弯17563@6m/s三组都完成，但CARLA/TCP/pursuit独立巡航速度RMS分别为.663/.922/.636m/s，均未通过.5门槛。
原四条开发路线没有覆盖真正的交替曲率，补充结果纠正了覆盖范围，而不是把已失败的样本排除。

### 第二轮闭环反馈迭代（用户已授权，2026-09-22晚）

用户指出首轮收尾不等于优化完成；10–15分钟仅是第一轮归档预计时间。下一轮实现、开发验证、冻结后回归暂估45–90分钟，
每个阶段完成即回报实际结果。先保留v1全部数据；v2在独立worktree开发，不改旧run或指标。

1. 修复route adapter的ego原点至首点速度膨胀。轨迹真正从估计后轴位姿出发，以几何重接路径的弧长定时；
   巡航速度、减速参数、控制gain不变。原dense route仍是独立误差参考，不能只报对新生成轨迹的误差。
2. 单测与旧传感器姿态重放验证速度界限、镜像、终点、退化几何。重放只验证接口/几何，不能代替CARLA闭环。
3. 冻结adapter后，在原四条G2@8m/s及17563@6m/s各跑CARLA/TCP/pursuit原配置，单列新旧adapter差异。
   同一adapter下再比较pursuit的additive/max前视规则；不同时调整gain或纵向速度，不按Dev10单路线特判。
4. 参数只用无交互开发集选择。两规则都合格时需路线等权横向RMS改善≥20%、无新增gate失败、任一路线p95恶化≤.1m、
   速度RMS恶化≤.1m/s才列替换候选；仅一规则全通过时如实说明资格差异。追加复跑须明确固定的随机性来源。
5. 开发失败先分析残余轨迹/定位/执行问题，保留全部中间记录；不因完成路线就宣称通过，也不无限扩展参数搜索。
   合格候选冻结后再跑集成与完整配对Dev10回归。既有六条保留路线已见过结果，再用时必须叫复验，不能冒称盲测。

最终分数收益仍需固定真实planner的后续对照。本轮若没有通过门槛的默认，交付明确失败原因与下一步，不降低验收门槛。
