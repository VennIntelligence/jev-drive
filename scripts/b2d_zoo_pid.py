# Vendored verbatim (below the marker) from Bench2DriveZoo, branch uniad/vad, commit 498c1f799dd90faf840dedb3f0d3234ec2e567db,
# team_code/pid_controller.py: the trajectory -> VehicleControl PID that the Bench2DriveZoo UniAD / VAD agents
# (and, with the same gains and window semantics, TCP / AD-MLP) ship. Only this header is added; do not edit below.
# Its RoutePlanner (team_code/planner.py, identical on both branches up to whitespace) is used as shipped too, from
# the checkout at $BENCH2DRIVE_ZOO_ROOT or vendored in scripts/b2d_zoo_planner.py.
# ---- vendored ----
