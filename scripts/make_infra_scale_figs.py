...
SERIES = {  # step -> (label, colour, line style)
    ...
    "t13-alp": ("Town13, Alpamayo rig", "#D55E00", "-"),
    "t13-alp-lights": ("Town13, Alpamayo rig, cached lights", "#E69F00", "--"),
    "t03-alp": ("Town03, Alpamayo rig", "#009E73", "-"),
    "t12-op": ("Town12, openpilot+TCP rig", "#CC79A7", "-"),
}
...
def load(runs: Path) -> pd.DataFrame:
    ...
    for step in SERIES:
        ...
        for line in f.read_text().splitlines():
            ...
            rows.append(dict(
                step=step, gpus=len(r["gpus"]), servers_per_gpu=r["servers_per_gpu"], workers=r["workers"],
...
def throughput(df: pd.DataFrame, out: Path):
    with mpl.rc_context(STYLE):
        ...
        for step, (label, color, ls) in SERIES.items():
            ...
            if not len(d):
                continue
            axes[0].plot(d.servers_per_gpu, d.agg_ticks_s, ls, marker="o", color=color, label=label)
            one = d[d.servers_per_gpu == 1]
            if len(one):
                n = d.servers_per_gpu
                axes[0].plot(n, one.agg_ticks_s.iloc[0] * n, ":", color=color, lw=0.6)
            axes[1].plot(d.servers_per_gpu, d.ms_per_tick_worker, ls, marker="o", color=color, label=label)
        ...
        fig.legend(h, l, loc="upper center", bbox_to_anchor=(0.5, 0.0), ncol=3)
...
def resources(df: pd.DataFrame, out: Path):
    with mpl.rc_context(STYLE):
        ...
        for step, (label, color, ls) in SERIES.items():
            ...
            if not len(d):
                continue
            axes[0].plot(d.servers_per_gpu, d.server_cores_each, ls, marker="o", color=color, label=label)
            axes[0].plot(d.servers_per_gpu, d.route_cores_each, ls, marker="^", color=color, alpha=0.6)
            axes[1].plot(d.servers_per_gpu, d.cores_mine, ls, marker="o", color=color)
            axes[2].plot(d.servers_per_gpu, d.gpu_util, ls, marker="o", color=color)
        axes[0].set(xlabel="CARLA servers on one GPU", ylabel="cores per process")
        axes[0].text(0.02, 0.98, "circles: server\ntriangles: route client", transform=axes[0].transAxes,
                     va="top", fontsize=6)
