...
#   scripts/seeds_overnight.sh score    the scoring part alone
...
do_score() {  # devkit scores of the NAVSIM seeds 1, 2 (late + ego heads) and of the cls heads' seed-0 re-run
  for s in 1 2; do
...
case $1 in
  ...
  seeds)
    ...
    do_score ;;
  score) do_score ;;      # scoring only (started beside a running `seeds` once its NAVSIM heads exist)
esac
